...
EOF
        ...
        packages = with pkgs; [
          ...
          '')
          (script "check-driver" ''
            # Check usb_free_urb declaration in usb-skeleton.c
            cd $REPO_ROOT/drivers
            
            echo "Checking usb_free_urb declaration in usb-skeleton.c..."
            if grep -q "usb_free_urb" usb-skeleton.c; then
              if ! grep -q "extern void usb_free_urb" usb-skeleton.c; then
                # Add the declaration if missing
                echo "Adding usb_free_urb declaration to usb-skeleton.c"
                sed -i '/static void launcher_draw_down(struct usb_launcher \*dev);/a\\n/* Forward declarations for kernel APIs */\\nextern void usb_free_urb(struct urb *urb);' usb-skeleton.c
              else
                echo "usb_free_urb is already declared in usb-skeleton.c"
              fi
            fi
            
            # Simple sanity check for source file
            if gcc -fsyntax-only -c usb-skeleton.c 2>&1 | grep -q "usb_free_urb"; then
