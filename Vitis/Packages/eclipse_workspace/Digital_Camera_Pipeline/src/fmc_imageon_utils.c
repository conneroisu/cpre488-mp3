...
void set_brightness(camera_config_t *config,
                    int percent //
) {
  if (config->bVerbose) {
...
void set_contrast(camera_config_t *config,
                  int percent //
) {
  if (config->bVerbose) {
...
/// @param config
/// @param percent
void set_saturation(camera_config_t *config,
                    int percent //
) {
  if (config->bVerbose) {
...
/// @brief increase_contrast increase the contrast of the camera
/// output
/// @param config the camera configuration
...
/// @brief decrease_contrast decrease the contrast of the camera
/// output
/// @param config the camera configuration
...
/// @brief increase_brightness increase the brightness of the camera
/// output
/// @param config the camera configuration
void increase_brightness(camera_config_t *config //
) {
  int brightness =
      XVprocSs_GetPictureBrightness(&proc_ss_RGB_YCrCb_444);
  brightness = clamp(brightness + INCR_DECR_VALUE);
  set_brightness(config, brightness);
}

/// @brief decrease_brightness decrease the brightness of the camera
/// output
/// @param config the camera configuration
...
/// @brief increase_saturation increase the saturation of the camera
/// output
/// @param config the camera configuration
void increase_saturation(camera_config_t *config //
) {
  int saturation =
      XVprocSs_GetPictureSaturation(&proc_ss_RGB_YCrCb_444);
  saturation = clamp(saturation + INCR_DECR_VALUE);
  set_saturation(config, saturation);
}

/// @brief decrease_saturation decrease the saturation of the camera
/// output
/// @param config the camera configuration
