...
void get_neighbors(t_location location, t_neighbors *neighbors) {
  ...
  for (int i = -1; i <= 1; ++i) {
    for (int j = -1; j <= 1; ++j) {
      ...
      t_location loc = {.row = location.row + i,
                        .col = location.col + j};
...
t_colors get_filter_color(t_location location) {
  ...
  return location.row % 2   ? location.col % 2 ? RED : GREEN
         : location.col % 2 ? GREEN
                            : BLUE;
}
...
void run_demosaicing(uint16_t *intensities, uint16_t *yuv_out) {
  ...
  int red_count, blue_count, green_count, red_total, blue_total,
      green_total, index = 0;
  uint8_t intensity = 0;
  ...
  for (int i = 0; i < HEIGHT; ++i) {
    ...
    for (int j = 0; j < WIDTH; j += 2) {
      ...
      t_color_24_bit rgb = (t_color_24_bit){
          .red = !red_count ? 0 : red_total / red_count,
          .green = !green_count ? 0 : green_total / green_count,
          .blue = !blue_count ? 0 : blue_total / blue_count};
...
uint32_t rgb_to_yuv(t_color_24_bit rgb) {
  ...
  y = (((uint16_t)rgb.red) * 0.183f) +
      (((uint16_t)rgb.green) * 0.614f) +
      (((uint16_t)rgb.blue) * 0.062f) + 16;
  v = (rgb.red * -0.101f) + (rgb.green * -0.338f) +
      (rgb.blue * 0.439f) + 128;
  u = (rgb.red * 0.439f) + (rgb.green * -0.399f) +
      (rgb.blue * -0.04f) + 128;

  // Set from YUV values.
  result = (uint32_t)((u << 8) | (y & 0xFF)) |
           ((uint32_t)((v << 8) | ((y & 0xFF00) >> 8)) << 16);
