...
/*****************************************************************************
 * camera_app.c - main camera application code. The camera configures
 *the various video in and video out peripherals, and (optionally)
 *performs some image processing on data coming in from the vdma.
 *
 *
 * NOTES:
...
void swap_frame_pointers(XAxiVdma *vdma, u16 dir, u8 a, u8 b) {
  u32 offset =
      XAXIVDMA_TX_OFFSET ? dir == XAXIVDMA_WRITE : XAXIVDMA_RX_OFFSET;

#define START_ADDR(index)                                            \
  *((volatile u32 *)(vdma->BaseAddr + offset +                       \
                     XAXIVDMA_START_ADDR_OFFSET + (index * 0x4)))
#define VSIZE                                                        \
  *((volatile u32 *)(vdma->BaseAddr + offset + XAXIVDMA_VSIZE_OFFSET))
...
void video_frame_output_isr(void *CallBackRef, u32 InterruptTypes) {
  ...
  case XAXIVDMA_IXR_FRMCNT_MASK: {
    // Once we have read from the back buffer, we know that it is the
    // new front buffer so we must have just swapped.
    if (sw_mode &&
        (get_current_frame_pointer((XAxiVdma *)CallBackRef,
                                   XAXIVDMA_READ) == target_frame) &&
        snapshot_saved) {
      // We start and stop the timer on this ISR.
      // If this is the first frame, the timer won't be started, so
      // start it up without ending any timer.
      if (!tStart) {
...
void camera_config_init(camera_config_t *config) {
  ...
  // config->uBaseAddr_TPG_PatternGenerator =
  // XPAR_V_TPG_0_S_AXI_CTRL_BASEADDR; // TPG Device
...
// Main (SW) processing loop. Recommended to have an explicit exit
// condition
void camera_loop(camera_config_t *config) {
  ...
  // Grab the DMA parkptr, and update it to ensure that when parked,
  // the S2MM side is on frame 1, and the MM2S side on frame 2
  ...
  vdma_MM2S_DMACR =
      XAxiVdma_ReadReg(config->vdma_hdmi.BaseAddr,
  ...
  volatile Xuint16 *pMM2S_Mem = (Xuint16 *)XAxiVdma_ReadReg(
      config->vdma_hdmi.BaseAddr, XAXIVDMA_MM2S_ADDR_OFFSET +
                                      XAXIVDMA_START_ADDR_OFFSET +
                                      (front_buffer_frame * 0x4));
  ...
  for (int j = 0; j < 1000; j++) {
    ...
    while (get_current_frame_pointer(&(config->vdma_hdmi),
                                     XAXIVDMA_WRITE)) {
    }
    ...
    set_park_frame(&(config->vdma_hdmi), front_buffer_frame,
                   XAXIVDMA_READ);
    ...
    pMM2S_Mem = (Xuint16 *)XAxiVdma_ReadReg(
        config->vdma_hdmi.BaseAddr, XAXIVDMA_MM2S_ADDR_OFFSET +
                                        XAXIVDMA_START_ADDR_OFFSET +
                                        (back_buffer_frame * 0x4));
  }
