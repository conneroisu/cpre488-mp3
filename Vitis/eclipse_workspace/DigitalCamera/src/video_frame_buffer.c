...
/*****************************************************************************
 * video_frame_buffer.c - VDMA code. Uses higher-level AXI-VDMA
 *functions (as opposed to explicitly writing register values) to
 *configure the triple framebuffer VDMA mode.
 *
 *
 * NOTES:
...
int vfb_common_init(u16 uDeviceId, XAxiVdma *pAxiVdma) {
  ...
  Status =
      XAxiVdma_CfgInitialize(pAxiVdma, Config, Config->BaseAddress);
  ...
  XScuGic_SetPriorityTriggerType(
      &int_controller, XPAR_FABRIC_AXI_VDMA_0_MM2S_INTROUT_INTR, 0xA0,
      0x3);
  XScuGic_SetPriorityTriggerType(
      &int_controller, XPAR_FABRIC_AXI_VDMA_0_S2MM_INTROUT_INTR, 0xA0,
      0x3);
  ...
  status = XScuGic_Connect(
      &int_controller, XPAR_FABRIC_AXI_VDMA_0_MM2S_INTROUT_INTR,
      (Xil_InterruptHandler)XAxiVdma_ReadIntrHandler, pAxiVdma);
  if (status != XST_SUCCESS) {
    xil_printf(
        "ERROR: GIC could not connect the VDMA write interrupt!\n\r");
  }

  status = XScuGic_Connect(
      &int_controller, XPAR_FABRIC_AXI_VDMA_0_S2MM_INTROUT_INTR,
      (Xil_InterruptHandler)XAxiVdma_WriteIntrHandler, pAxiVdma);
  if (status != XST_SUCCESS) {
    xil_printf(
        "ERROR: GIC could not connect the VDMA read interrupt!\n\r");
  }

  // Enable interrupt on GIC
  XScuGic_Enable(&int_controller,
...
int vfb_rx_init(XAxiVdma *pAxiVdma,
  ...
  if (Status != XST_SUCCESS) {
    xdbg_printf(XDBG_DEBUG_ERROR, "Write channel setup failed %d\r\n",
                Status);
  ...
  XAxiVdma_SetCallBack(pAxiVdma, XAXIVDMA_HANDLER_GENERAL,
                       video_frame_output_isr, (void *)pAxiVdma,
                       XAXIVDMA_READ);
  XAxiVdma_SetCallBack(pAxiVdma, XAXIVDMA_HANDLER_ERROR, error_isr,
                       (void *)pAxiVdma, XAXIVDMA_READ);
  XAxiVdma_IntrEnable(pAxiVdma, XAXIVDMA_IXR_FRMCNT_MASK,
                      XAXIVDMA_READ);
...
int vfb_tx_init(XAxiVdma *pAxiVdma,
  ...
  // Configure RX int handler.
  XAxiVdma_SetCallBack(pAxiVdma, XAXIVDMA_HANDLER_GENERAL,
                       camera_input_isr, (void *)pAxiVdma,
                       XAXIVDMA_WRITE);
  XAxiVdma_SetCallBack(pAxiVdma, XAXIVDMA_HANDLER_ERROR, error_isr,
                       (void *)pAxiVdma, XAXIVDMA_WRITE);
  XAxiVdma_IntrEnable(pAxiVdma, XAXIVDMA_IXR_FRMCNT_MASK,
                      XAXIVDMA_WRITE);
...
int vfb_dump_registers(XAxiVdma *pAxiVdma) {
  ...
  xil_printf(
      "AXI_VDMA - Partial Register Dump (uBaseAddr = 0x%08X):\n\r",
      uBaseAddr);
  xil_printf(
      "\t PARKPTR          = 0x%08X\n\r",
  ...
  xil_printf("\t MM2S_DMASR       = 0x%08X\n\r",
             *((volatile int *)(uBaseAddr + XAXIVDMA_TX_OFFSET +
                                XAXIVDMA_SR_OFFSET)));
  xil_printf(
      "\t MM2S_STRD_FRMDLY = 0x%08X\n\r",
      *((volatile int *)(uBaseAddr + XAXIVDMA_MM2S_ADDR_OFFSET +
                         XAXIVDMA_STRD_FRMDLY_OFFSET)));
  xil_printf(
      "\t MM2S_START_ADDR0 = 0x%08X\n\r",
      *((volatile int *)(uBaseAddr + XAXIVDMA_MM2S_ADDR_OFFSET +
                         XAXIVDMA_START_ADDR_OFFSET + 0)));
  xil_printf(
      "\t MM2S_START_ADDR1 = 0x%08X\n\r",
      *((volatile int *)(uBaseAddr + XAXIVDMA_MM2S_ADDR_OFFSET +
                         XAXIVDMA_START_ADDR_OFFSET + 4)));
  xil_printf(
      "\t MM2S_START_ADDR2 = 0x%08X\n\r",
      *((volatile int *)(uBaseAddr + XAXIVDMA_MM2S_ADDR_OFFSET +
                         XAXIVDMA_START_ADDR_OFFSET + 8)));
  xil_printf(
      "\t MM2S_HSIZE       = 0x%08X\n\r",
...
int vfb_check_errors(XAxiVdma *pAxiVdma, u8 bClearErrors) {
  ...
  // inErrors = XAxiVdma_GetStatus(pAxiVdma, XAXIVDMA_WRITE) &
  // XAXIVDMA_SR_ERR_ALL_MASK; outErrors =
  // XAxiVdma_GetStatus(pAxiVdma, XAXIVDMA_READ) &
