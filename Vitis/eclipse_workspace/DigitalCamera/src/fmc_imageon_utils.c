...
/*****************************************************************************
 * fmc_imageon_utils.c - main initialization and configuration
 *routines for the Avnet FMC-IMAGEON board. Most of the bugs are their
 *fault.
 *
 *
 * NOTES:
...
int fmc_imageon_enable(camera_config_t *config) {
  ...
  if (config->bVerbose) {
    ...
    xil_printf("\tVideo Dimensions = %d x %d\n\r",
               config->hdmio_width, config->hdmio_height);
  }
  ...
  onsemi_vita_spi_config(
      &(config->onsemi_vita),
      (75000000 /
       10000000)); // AXI-Lite SPI Speed (HZ) / 10,000,000 Hz
  ...
  // Uncomment to enable HW Video processing pipeling (last part of
  // lab) You need to complete implmentation of this function before
  // enabling
  //   fmc_imageon_enable_ipipe(config);

  // Output Video input source in Hardware mode for 10 seconds
  xil_printf("Output Video input source in Hardware mode for 10 "
             "seconds\n\r");
  // sleep(10);
...
int fmc_imageon_enable_tpg(camera_config_t *config) {
  ...
  volatile uint32_t *TPG_CR =
      (volatile uint32_t *)(config->uBaseAddr_TPG_PatternGenerator +
                            0); // TPG Control
  volatile uint32_t *TPG_Act_H =
      (volatile uint32_t *)(config->uBaseAddr_TPG_PatternGenerator +
                            0x10); // Active Height
  volatile uint32_t *TPG_Act_W =
      (volatile uint32_t *)(config->uBaseAddr_TPG_PatternGenerator +
                            0x18); // Active Width
  volatile uint32_t *TPG_BGP =
      (volatile uint32_t *)(config->uBaseAddr_TPG_PatternGenerator +
                            0x20); // Background Pattern
  volatile uint32_t *TPG_FGP =
      (volatile uint32_t *)(config->uBaseAddr_TPG_PatternGenerator +
                            0x28); // Foreground Pattern
  volatile uint32_t *TPG_MS =
      (volatile uint32_t *)(config->uBaseAddr_TPG_PatternGenerator +
                            0x38); // Motion Speed
  volatile uint32_t *TPG_CF =
      (volatile uint32_t *)(config->uBaseAddr_TPG_PatternGenerator +
                            0x40); // TPG Color Format
  volatile uint32_t *TPG_BOX_SIZE =
      (volatile uint32_t *)(config->uBaseAddr_TPG_PatternGenerator +
                            0x78);
  volatile uint32_t *TPG_BOX_COLOR_Y =
      (volatile uint32_t *)(config->uBaseAddr_TPG_PatternGenerator +
                            0x80);
  volatile uint32_t *TPG_BOX_COLOR_U =
      (volatile uint32_t *)(config->uBaseAddr_TPG_PatternGenerator +
                            0x88);
  volatile uint32_t *TPG_BOX_COLOR_V =
      (volatile uint32_t *)(config->uBaseAddr_TPG_PatternGenerator +
                            0x90);
  ...
  // (UNDER CONSTRUCTION: DO NOT USE!!) Useful portable wrapper
  // functions for accessing TPG configuration registers
  /*
  // See TPG data sheet for configuring the TPG for other features

  xTPG_config(config->hdmio_width, config->hdmio_height,
  config->uBaseAddr_TPG_PatternGenerator, 1);

  // Default to Zone Plate
  Reg32Value =
  XTPG_mReadSlaveReg0(config->uBaseAddr_TPG_PatternGenerator, 0);
  XTPG_mWriteSlaveReg0(config->uBaseAddr_TPG_PatternGenerator, 0,
  (Reg32Value & 0xFFFFFFF0) | 10); ZPlate_config(config->hdmio_width,
  config->hdmio_height, config->uBaseAddr_TPG_PatternGenerator);

  // Enable motion
  Reg32Value =
  XTPG_mReadSlaveReg1(config->uBaseAddr_TPG_PatternGenerator, 0); if
  ((Reg32Value & 0x00000001)==0)
          XTPG_mWriteSlaveReg1(config->uBaseAddr_TPG_PatternGenerator,
  0, (Reg32Value | 0x00000001)); else
          XTPG_mWriteSlaveReg1(config->uBaseAddr_TPG_PatternGenerator,
  0, (Reg32Value & 0xFFFFFFFE));

  // Speed = 4
  Reg32Value =
  XTPG_mReadSlaveReg1(config->uBaseAddr_TPG_PatternGenerator, 0);
  CurrentSpeed = (Reg32Value&0x000001fe)>>1;
  CurrentSpeed = 4;
  XTPG_mWriteSlaveReg1(config->uBaseAddr_TPG_PatternGenerator, 0,
  (CurrentSpeed<<1)|(Reg32Value & 0x00000001));
...
int fmc_imageon_disable_tpg(camera_config_t *config) {
  ...
  // Define convenient volatile pointers for accessing TPG registers
  volatile unsigned int *TPG_CR =
      config->uBaseAddr_TPG_PatternGenerator + 0; // TPG Control
...
// XVprocSs proc_ss_RGB_YCrCb_444;  // To hold info for the Video
// Processing Subsystem: Color Conversion Only IP core: See
// xv_procss.h, and xv_csc_l2.h XVprocSs_Config *Config_ptr;
...
int fmc_imageon_enable_ipipe(camera_config_t *config) {
  ...
  // register details. Hint 1: You will need to configure 4 additional
  // registers, Hint 2: You will need to dig through some header files
  // for some of the values, Hint 3: Set up the Demosaic IP block
  // first. It is a good warm up.

  // Add assignments here

  // Uncomment as part of Re-sampler IP setup
  //   Xil_Out32((XPAR_V_PROC_SS_2_BASEADDR) +
  //   (XV_HCRESAMPLER_CTRL_ADDR_AP_CTRL), (u32)(0x81));  // Control
  xil_printf("4:4:4 to 4:2:2 Re-sampling IP Configuration and Enable "
             "done\r\n");

  // Video Processing Subsystem Hardware IP (configured for Only Color
  // Conversion) from 24-bit RGB to YCrCb 4:4:4 For this IP core there
  // was a High level API for setting it up.  Trace through these
  // calls to see how much work they do. This could have been set up
  // with direct register writes, but there are about 20 registers
  // that need to be set for this IP block

  // Uncomment to setup Color Conversion IP
  //   Config_ptr = XVprocSs_LookupConfig(XPAR_XVPROCSS_0_DEVICE_ID);
  //   XVprocSs_CfgInitialize(&proc_ss_RGB_YCrCb_444, Config_ptr,
  ...
  //   XVIDC_CSF_RGB, XVIDC_CSF_YCRCB_444, XVIDC_BT_709, XVIDC_BT_709,
  //   XVIDC_CR_0_255); XVprocSs_Start(&proc_ss_RGB_YCrCb_444);
  xil_printf("RGB to 4:4:4 IP Configuration and Enable done\r\n");

  // Demosaic to convert sensor Bayer pattern into 24-bit RGB
  // TODO Add additional register assignments here to fully configure
  // this core. See Demosaic IP documentation for register details.
  ...
  //   (XV_DEMOSAIC_CTRL_ADDR_AP_CTRL), (u32)(0x81));// 0b10000001
  //   means start and freerun mode (page 16 in PG286)
  xil_printf(
