 ...
 * ----- ---- --------
 *------------------------------------------------------- 1.00a ecm
 *01/10/10 Initial release 3.00a mb  25/06/12 InitQspi, data is read
 *first and required config bits are set 4.00a sg	02/28/13
 *Cleanup Removed LPBK_DLY_ADJ register setting code as we use divisor
 *8 5.00a sgd	05/17/13 Added Flash Size > 128Mbit support Dual Stack
 *support Fix for CR:721674 - FSBL- Failed to boot from Dual stacked
 *QSPI 6.00a kc  08/30/13 Fix for CR#722979 - Provide
 *customer-friendly changelogs in FSBL Fix for CR#739711 - FSBL not
 *able to read Large QSPI (512M) in IO Mode 7.00a kc  10/25/13 Fix for
 *CR#739968 - FSBL should do the QSPI config settings for Dual
 *parallel configuration in IO mode 14.0 gan 01/13/16  Fix for
 *CR#869081 - (2016.1)FSBL picks the qspi read command from LQSPI_CFG
 *register instead of hard coded read command (0x6B).
 *
 * </pre>
 *
 * @note
 ...
 * xparameters.h file. They are defined here such that a user can
 * easily change all the needed parameters in one place.
 */
#define QSPI_DEVICE_ID XPAR_XQSPIPS_0_DEVICE_ID

/*
 * The following constants define the commands which may be sent to
 * the FLASH device.
 */
...
#define ADDRESS_1_OFFSET 1 /* MSB byte of address to read or write   \
                            */
#define ADDRESS_2_OFFSET                                             \
  2 /* Middle byte of address to read or write */
...
#define BANK_SEL_SIZE                                                \
  2 /* BRWR or EARWR command + 1 byte bank value */
#define WRITE_ENABLE_CMD_SIZE 1 /* WE command */
/*
 * The following constants specify the extra bytes which are sent to
 * the FLASH on the QSPI interface, that are not data, but control
 * information which includes the command and address
 */
#define OVERHEAD_SIZE 4

/*
 * The following constants specify the max amount of data and the size
 * of the the buffer required to hold the data and overhead to
 * transfer the data to and from the FLASH.
 */
...
#define SINGLE_QSPI_CONFIG_FAST_READ                                 \
  (XQSPIPS_LQSPI_CR_LINEAR_MASK | LQSPI_CR_1_DUMMY_BYTE |            \
   LQSPI_CR_FAST_READ)

#define SINGLE_QSPI_CONFIG_FAST_DUAL_READ                            \
  (XQSPIPS_LQSPI_CR_LINEAR_MASK | LQSPI_CR_1_DUMMY_BYTE |            \
   LQSPI_CR_FAST_DUAL_READ)

#define SINGLE_QSPI_CONFIG_FAST_QUAD_READ                            \
  (XQSPIPS_LQSPI_CR_LINEAR_MASK | LQSPI_CR_1_DUMMY_BYTE |            \
   LQSPI_CR_FAST_QUAD_READ)

#define DUAL_QSPI_CONFIG_FAST_QUAD_READ                              \
  (XQSPIPS_LQSPI_CR_LINEAR_MASK | XQSPIPS_LQSPI_CR_TWO_MEM_MASK |    \
   XQSPIPS_LQSPI_CR_SEP_BUS_MASK | LQSPI_CR_1_DUMMY_BYTE |           \
   LQSPI_CR_FAST_QUAD_READ)

#define DUAL_STACK_CONFIG_FAST_READ                                  \
  (XQSPIPS_LQSPI_CR_TWO_MEM_MASK | LQSPI_CR_1_DUMMY_BYTE |           \
   LQSPI_CR_FAST_READ)

#define DUAL_STACK_CONFIG_FAST_DUAL_READ                             \
  (XQSPIPS_LQSPI_CR_TWO_MEM_MASK | LQSPI_CR_1_DUMMY_BYTE |           \
   LQSPI_CR_FAST_DUAL_READ)

#define DUAL_STACK_CONFIG_FAST_QUAD_READ                             \
  (XQSPIPS_LQSPI_CR_TWO_MEM_MASK | LQSPI_CR_1_DUMMY_BYTE |           \
   LQSPI_CR_FAST_QUAD_READ)

#define SINGLE_QSPI_IO_CONFIG_FAST_READ                              \
  (LQSPI_CR_1_DUMMY_BYTE | LQSPI_CR_FAST_READ)

#define SINGLE_QSPI_IO_CONFIG_FAST_DUAL_READ                         \
  (LQSPI_CR_1_DUMMY_BYTE | LQSPI_CR_FAST_DUAL_READ)

#define SINGLE_QSPI_IO_CONFIG_FAST_QUAD_READ                         \
  (LQSPI_CR_1_DUMMY_BYTE | LQSPI_CR_FAST_QUAD_READ)

#define DUAL_QSPI_IO_CONFIG_FAST_QUAD_READ                           \
  (XQSPIPS_LQSPI_CR_TWO_MEM_MASK | XQSPIPS_LQSPI_CR_SEP_BUS_MASK |   \
   LQSPI_CR_1_DUMMY_BYTE | LQSPI_CR_FAST_QUAD_READ)
...
/*
 * The following variables are used to read and write to the eeprom
 * and they are global to avoid having large buffers on the stack
 */
 ...
 *and it presently supports Micron and Spansion - 128, 256 and 512Mbit
 *and Winbond 128Mbit
 *
 * @param	none
 *
 ...
 * QSPI interface.
 *
 * @param	Address contains the address to read data from in the
 *FLASH.
...
/**
 *
 * This function provides the QSPI FLASH interface for the Simplified
 ...
 *		- XST_FAILURE if the write fails to completes
 *correctly
 *
 * @note	none.
 *
 ...
 *
 * @param	BankSel is the bank to be selected in the flash
 *device(s).
 *
