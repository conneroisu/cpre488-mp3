...
u32 AuthenticatePartition(u8 *Buffer, u32 Size) {
  ...
  rsa2048_pubexp((RSA_NUMBER)DecryptSignature,
                 (RSA_NUMBER)SignaturePtr, (u32)PpkExp,
                 (RSA_NUMBER)PpkModular, (RSA_NUMBER)PpkModularEx);
   ...
   */
  rsa2048_pubexp((RSA_NUMBER)DecryptSignature,
                 (RSA_NUMBER)SignaturePtr, (u32)SpkExp,
                 (RSA_NUMBER)SpkModular, (RSA_NUMBER)SpkModularEx);
  ...
  sha_256((u8 *)Buffer, (Size - RSA_PARTITION_SIGNATURE_SIZE),
          HashSignature);
  FsblPrintArray(HashSignature, 32, "Partition Hash Calculated");
 ...
 * @param	Partition hash value which includes boot header,
 *partition data
 * @return
...
u32 RecreatePaddingAndCheck(u8 *signature, u8 *hash) {
  u8 T_padding[] = {0x30, 0x31, 0x30, 0x0D, 0x06, 0x09, 0x60,
   ...
   * MSB  ----------------------------------------------------LSB
   * 0x0 || 0x1 || 0xFF(for 202 bytes) || 0x0 || T_padding || SHA256
   * Hash
   */
