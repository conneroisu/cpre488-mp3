 ...
 * @file nand.c
 *
 * Contains code for the NAND FLASH functionality. Bad Block
 *management is simple: skip the bad blocks and keep going.
 *
 * <pre>
 ...
 * ----- ---- --------
 *------------------------------------------------------- 1.00a ecm
 *01/10/10 Initial release 2.00a  mb	25/05/12 fsbl changes for
 *standalone bsp based 3.00a sgd	30/01/13 Code cleanup 5.00a
 *sgd	17/05/13 Support for Multi Boot
 * </pre>
 *
 * @note
 *
 ...
 * ******************************/
static u32
XNandPs_CalculateLength(XNandPs *NandInstPtr, u64 Offset, u32 Length);
...
/**
 *
 * This function initializes the controller for the NAND FLASH
 *interface.
 *
 * @param	none
 *
 ...
 *		- XST_FAILURE if the controller fails to initializes
 *correctly
 *
 * @note		none.
 *
...
u32 InitNand(void) {
  ...
  Status =
      XNandPs_CfgInitialize(NandInstPtr, ConfigPtr,
...
/**
 *
 * This function provides the NAND FLASH interface for the Simplified
 *header functionality. This function handles bad blocks.
 *
 * The source address is the absolute good address, bad blocks are
 *skipped without incrementing the source address.
 *
 * @param	SourceAddress is address in FLASH data space, absolute
 *good address
 * @param	DestinationAddress is address in OCM data space
 *
 * @return	XST_SUCCESS if the transfer completes correctly
 *		XST_FAILURE if the transfer fails to completes
 *correctly
 *
 * @note	none.
 *
...
/**
 *
 * This function returns the length including bad blocks from a given
 *offset and length.
 *
