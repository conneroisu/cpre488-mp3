 ...
 * ----- ---- --------
 *------------------------------------------------------- 1.00a ecm
 *02/10/10	Initial release 2.00a jz	05/28/11	Add SD
 *support 2.00a mb	25/05/12	using the EDK provided devcfg
 *driver Nand/SD encryption and review comments 3.00a mb  16/08/12
 *Added the poll function Removed the FPGA_RST_CTRL define Added the
 *flag for NON PS instantiated bitstream 4.00a sgd 02/28/13	Fix
 *for CR#681014 - ECC init in FSBL should not call fabric_init() Fix
 *for CR#689026 - FSBL doesn't hold PL resets active during bit
 *download Fix for CR#699475 - FSBL functionality is broken and its
 *not able to boot in QSPI/NAND bootmode Fix for CR#705664 - FSBL
 *fails to decrypt the bitstream when the image is AES encrypted using
 *non-zero key value 6.00a kc  08/30/13    Fix for CR#722979 - Provide
 *customer-friendly changelogs in FSBL 7.00a kc	10/25/13	Fix
 *for CR#724620 - How to handle PCAP_MODE after bitstream
 *configuration Fix for CR#726178 - FabricInit() PROG_B is kept active
 *						                    for
 *5mS. Fix for CR#731839 - FSBL has to check the HMAC error status
 *after decryption 12/04/13	Fix for CR#764382 - How to handle
 *PCAP_MODE after bitstream configuration - PCAP_MODE and PCAP_PR bits
 *are not modified 8.00a kc  2/20/14		Fix for CR#775631 -
 *FSBL: FsblGetGlobalTimer() is not proper 10.00a kc 07/24/14    Fix
 *for CR#809336 - Minor code cleanup 13.00a ssc 04/10/15   Fix for
 *CR#846899 - Corrected logic to clear DMA done count 15.00a gan
 *07/21/16   Fix for CR# 953654 -(2016.3)FSBL - In
 *pcap.c/pcap.h/main.h, Fabric Initialization sequence is modified to
 *check the PL power before sequence starts and checking INIT_B reset
 *status twice in case of failure. 16.00a gan 08/02/16   Fix for CR#
 *955897 -(2016.3)FSBL - In pcap.c, check pl power through MCTRL
 *register for 3.0 and later versions of silicon.
 * </pre>
 *
 * @note
 *
 ...
 *
 * @param 	SourceDataPtr is a pointer to where the data is read
 *from
 * @param 	DestinationDataPtr is a pointer to where the data is
 *written to
 * @param 	SourceLength is the length of the data to be moved in
 *words
 * @param 	DestinationLength is the length of the data to be
 *moved in words
 * @param 	SecureTransfer indicated the encryption key location,
 *0 for non-encrypted
 *
 * @return
 ...
 *
 ****************************************************************************/
u32 PcapDataTransfer(u32 *SourceDataPtr,
  ...
  DestinationDataPtr =
      (u32 *)((u32)DestinationDataPtr | PCAP_LAST_TRANSFER);
 ...
 *
 ****************************************************************************/
u32 PcapLoadPartition(u32 *SourceDataPtr,
                      u32 *DestinationDataPtr,
                      u32 SourceLength,
                      u32 DestinationLength,
                      u32 SecureTransfer) {
 ...
 *
 * @return
 *		- XST_SUCCESS if the pcap driver initialization is
 *successful
 ...
 *
 * @return
 *		- XST_SUCCESS if the Fabric  initialization is
 *successful
...
u32 FabricInit(void) {
   ...
   */
  PcapReg =
      XDcfg_ReadReg(DcfgInstPtr->Config.BaseAddr, XDCFG_CTRL_OFFSET);
  ...
  if (TimerExpired == 1) {
    ...
    /*
     * Polling the PCAP_INIT status for Reset or timeout (second
     * iteration)
     */
...
u32 ClearPcapStatus(void) {
  ...
  if ((StatusReg & XDCFG_STATUS_DMA_DONE_CNT_MASK) != 0) {
    XDcfg_SetStatusRegister(
        DcfgInstPtr, StatusReg | XDCFG_STATUS_DMA_DONE_CNT_MASK);
  }
 ...
 *
 * @return
 *		- XST_SUCCESS if polling for DMA/FPGA done is
 *successful
