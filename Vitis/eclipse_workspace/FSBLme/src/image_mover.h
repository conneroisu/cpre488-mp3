...
* @file image_mover.h
*
* This file contains the interface for moving the image from FLASH to
OCM
...
* 5.00a kc	07/30/13	Added defines for image header
information
* 8.00a kc	01/16/13	Added defines for partition owner
attribute
* </pre>
*
* @note
...
#define TOTAL_PARTITION_HEADER_SIZE                                  \
  (MAX_PARTITION_NUMBER * IMAGE_HEADER_SIZE)
#define TOTAL_IMAGE_HEADER_SIZE                                      \
  (MAX_PARTITION_NUMBER * IMAGE_HEADER_SIZE)
#define TOTAL_HEADER_SIZE                                            \
  (IMAGE_HEADER_TABLE_SIZE + TOTAL_IMAGE_HEADER_SIZE +               \
   TOTAL_PARTITION_HEADER_SIZE + 64)

/* Partition Header defines */
#define PARTITION_IMAGE_WORD_LEN_OFFSET                              \
  0x00 /* Word length of image */
#define PARTITION_DATA_WORD_LEN_OFFSET                               \
  0x04 /* Word length of data                                        \
        */
#define PARTITION_WORD_LEN_OFFSET                                    \
  0x08 /* Word length of partition                                   \
        */
#define PARTITION_LOAD_ADDRESS_OFFSET 0x0C /* Load addr in DDR	*/
#define PARTITION_EXEC_ADDRESS_OFFSET                                \
  0x10                                  /* Addr to start executing */
#define PARTITION_ADDR_OFFSET 0x14      /* Partition word offset */
#define PARTITION_ATTRIBUTE_OFFSET 0x18 /* Partition type */
#define PARTITION_HDR_CHECKSUM_OFFSET                                \
  0x3C /* Header Checksum offset */
#define PARTITION_HDR_CHECKSUM_WORD_COUNT                            \
  0xF                                 /* Checksum word count */
...
#define ATTRIBUTE_RSA_PRESENT_MASK                                   \
  0x8000 /* RSA Signature Present                                    \
          */
#define ATTRIBUTE_PARTITION_OWNER_MASK 0x30000 /* Partition Owner */

#define ATTRIBUTE_PARTITION_OWNER_FSBL                               \
  0x00000 /* FSBL Partition Owner */
