 ...
 * ----- ---- --------
 *------------------------------------------------------- 1.00a jz
 *06/04/11	Initial release 2.00a mb	25/05/12
 *standalone based FSBL 3.00a np/mb	08/03/12	Added call to
 *FSBL user hook - before handoff. DDR ECC initialization added fsbl
 *print with verbose added Performance measurement added Flushed the
 *UART Tx buffer Added the performance time for ECC DDR init Added
 *clearing of ECC Error Code Added the watchdog timer value 4.00a sgd
 *02/28/13	Code Cleanup Fix for CR#681014 - ECC init in FSBL
 *should not call fabric_init() Fix for CR#689077 - FSBL hangs at
 *Handoff clearing the TX UART buffer when using UART0 instead of
 *UART1 Fix for CR#694038 - FSBL debug logs always prints 14.3 as the
 *Revision number - this is incorrect Fix for CR#694039 - FSBL prints
 *"unsupported silicon version for v3.0" 3.0 Silicon Fix for CR#699475
 *- FSBL functionality is broken and its not able to boot in QSPI/NAND
 *                                           bootmode
 *                       Removed DDR initialization check
 *                       Removed DDR ECC initialization code
 *						Modified hand off
 *address check to 1MB Added RSA authentication support Watchdog
 *disabled for AES E-Fuse encryption 5.00a sgd 05/17/13	Fallback
 *support for E-Fuse encryption Fix for CR#708728 - Issues seen while
 *making HP interconnect 32 bit wide 6.00a kc  07/30/13    Fix for
 *CR#708316 - PS7_init.tcl file should have Error mechanism for all
 *mask_poll Fix for CR#691150 - ps7_init does not check for peripheral
 *initialization failures or timeout on polls Fix for CR#724165 -
 *Partition Header used by FSBL is not authenticated Fix for CR#724166
 *- FSBL doesn’t use PPK authenticated by Boot ROM for authenticating
 *                                           the Partition images
 *                       Fix for CR#722979 - Provide customer-friendly
 *                                           changelogs in FSBL
 *                       Fix for CR#732865 - Backward compatibility
 *for ps7_init function 7.00a kc  10/18/13    Integrated SD/MMC driver
 * 8.00a kc  02/20/14	Fix for CR#775631 - FSBL: FsblGetGlobalTimer()
 *											is
 *not proper 9.00a kc  04/16/14	Fix for CR#724166 - SetPpk() will fail
 *on secure fallback unless FSBL* and FSBL are identical in length
 * 10.00a kc 07/24/14	Fix for CR#809336 - Minor code cleanup
 *        kc 08/27/14	Fix for CR#820356 - FSBL compilation fails
 *with IAR compiler 11.00a kv 10/08/14	Fix for CR#826030 -
 *LinearBootDeviceFlag should be initialized to 0 in IO mode case
 * 15.00a gan 07/21/16   Fix for CR# 953654 -(2016.3)FSBL -
 * 											In
 *pcap.c/pcap.h/main.c, Fabric Initialization sequence is modified to
 *check the PL power before sequence starts and checking INIT_B reset
 *status twice in case of failure. 16.00a bsv 03/26/18	Fix for CR#
 *996973  Add code under JTAG_ENABLE_LEVEL_SHIFTERS macro to enable
 *level shifters in jtag boot mode.
 * </pre>
 *
 * @note
 * FSBL runs from OCM, Based on the boot mode selected, FSBL will copy
 * the partitions from the flash device. If the partition is bitstream
 *then the bitstream is programmed in the Fabric and for an partition
 *that is an application , FSBL will copy the application into DDR and
 *does a handoff.The application should not be starting at the OCM
 *address, FSBL does not remap the DDR. Application should use DDR
 *starting from 1MB
 *
 * FSBL can be stitched along with bitstream and application using
 *bootgen
 *
 * Refer to fsbl.h file for details on the compilation flags supported
 *in FSBL
 *
 ******************************************************************************/
...
int main(void) {
  ...
  fsbl_printf(DEBUG_GENERAL, "Release %d.%d	%s-%s\r\n",
              SDK_RELEASE_YEAR, SDK_RELEASE_QUARTER, __DATE__,
              __TIME__);
   ...
   */
     ...
     * SD BOOT MODE
     */
#if defined(XPAR_PS7_SD_0_S_AXI_BASEADDR) ||                         \
    defined(XPAR_XSDPS_0_BASEADDR)
      ...
      if (BootModeRegister == JTAG_MODE) {
        ...
        /** If bitstream was loaded in jtag boot mode prior to running
         * FSBL */
...
void FsblHandoff(u32 FsblStartAddr) {
  ...
  if (BitstreamFlag) {
    /*
     * FSBL will not enable the level shifters for a NON PS
     * instantiated Bitstream CR# 671028 This flag can be set during
     * compilation for a NON PS instantiated bitstream
     */
    ...
    fsbl_printf(DEBUG_INFO,
                "Enabling Level Shifters PL to PS "
...
/**
 *
 * This function outputs the status for the provided State in the boot
 *process.
 *
 * @param	State is where in the boot process the output is
 *desired.
 *
 * @return	None.
 *
...
/**
 *
 * This function handles the error and lockdown processing and outputs
 *the status for the provided State in the boot process.
 *
 ...
 * @return	None.
 *
 * @note		This function does not return, the PS block is
 *reset
 *
 ****************************************************************************/
...
/**
 *
 * This function copies a string to another, the source string must be
 *null- terminated.
 *
...
static void RegisterHandlers(void) {
  ...
  Xil_ExceptionRegisterHandler(
      ...
      (Xil_ExceptionHandler)PreFetch_Abort_Handler, (void *)0);
  Xil_ExceptionRegisterHandler(
      XIL_EXCEPTION_ID_DATA_ABORT_INT,
      (Xil_ExceptionHandler)Data_Abort_Handler, (void *)0);
...
/**
 *
 * This function Updates the Multi boot Register to enable golden
 *image search for boot rom
 *
 * @param None
 *
 * @return
...
static void Update_MultiBootRegister(void) {
  ...
  if (Silicon_Version != SILICON_VERSION_1) {
    ...
    XDcfg_WriteReg(DcfgInstPtr->Config.BaseAddr,
                   XDCFG_MULTIBOOT_ADDR_OFFSET, MultiBootReg);
 ...
 *******************************************************************************/
#ifdef FSBL_PERF
void FsblGetGlobalTime(XTime *tCur) { XTime_GetTime(tCur); }
...
int InitWatchDog(void) {
  u32 Status = XST_SUCCESS;
  XWdtPs_Config
      *ConfigPtr; /* Config structure of the WatchDog Timer */
  u32 CounterValue = 1;
   ...
   */
  XWdtPs_SetControlValue(&Watchdog, XWDTPS_COUNTER_RESET,
                         CounterValue);
  /*
...
void CheckWDTReset(void) {
  ...
  /*
   *  For 1.0 Silicon the reason for Reset is in the ResetReason
   * Register Hence this register can be read to know the cause for
   * previous reset that happened. Check if that reset is a Software
   * WatchDog reset that happened
   */
  if (Silicon_Version == SILICON_VERSION_1) {
...
u32 ConvertTime_WdtCounter(u32 seconds) {
  ...
  time = (double)(PrescalerValue) /
         (double)XPAR_PS7_WDT_0_WDT_CLK_FREQ_HZ;
...
/******************************************************************************
 *
 * This function HeaderChecksum will calculates the header checksum
 *and compares with checksum read from flash
 *
...
u32 HeaderChecksum(u32 FlashOffsetAddress) {
  ...
  for (Count = 0; Count < IMAGE_HEADER_CHECKSUM_COUNT; Count++) {
     ...
     */
    MoveImage(FlashOffsetAddress + IMAGE_WIDTH_CHECK_OFFSET +
                  (Count * 4),
              (u32)&TempValue, 4);
   ...
   */
  Checksum ^= 0xFFFFFFFF;
  MoveImage(FlashOffsetAddress + IMAGE_CHECKSUM_OFFSET,
            (u32)&TempValue, 4);
