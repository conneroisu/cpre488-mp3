...
int vfb_check_errors(XAxiVdma *pAxiVdma, u8 bClearErrors) {
  ...
  outErrors =
      *((volatile int *)(uBaseAddr + 0x00000004)) & 0x000046F0;
...
void camera_config_init(camera_config_t *config) {
  config->uBaseAddr_IIC_FmcIpmi =
      0x41610000; // Device for reading HDMI board IPMI EEPROM
                  // information
  config->uBaseAddr_IIC_FmcImageon =
      0x41600000; // Device for configuring the HDMI board
  config->uBaseAddr_VITA_SPI =
      0x43C30000; // Device for configuring the Camera sensor
  config->uBaseAddr_VITA_CAM =
      0x43C20000; // Device for receiving Camera sensor data
  ...
  config->uNumFrames_HdmiFrameBuffer =
      0x5U; // Number of VDMA Frame buffers
  return;
}
