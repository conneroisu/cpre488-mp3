...
int fmc_imageon_enable(camera_config_t *config) {
  ...
  vres_get_timing(config->hdmio_resolution, &VideoTiming);
...
int camera_main() {
  ...
  res = fmc_imageon_enable(&camera_config);
  if (res != 0) {
    xil_printf("failed to enable imageon with imeagon_enable");
    return 1;
  }
