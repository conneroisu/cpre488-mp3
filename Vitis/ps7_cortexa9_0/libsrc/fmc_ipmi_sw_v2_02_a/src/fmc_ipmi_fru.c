...
//
//                      Currently supports:
//                      - Common Header (located at offset 0 in
//                      EEPROM)
//                      - Board Info Area (located at offset 8 in
//                      EEPROM)
//                      - Multi-Record Info Area (located at offset
//                      128 in EEPROM)
//
// Limitations/Issues:  Limitations:
//                      - Board Info Area always written to fixed
//                      offset of 8 bytes
//
//                      Not implemented yet (required as per FMC
//                      specification, Section 5.5.1):
//                      - Multi-Record Info:
//                         - OEM record type 0xFA (FMC specific VITA
//                         record)
//                         - DC Load for each of : VADJ, 2P2V, 12P0V
//                         - DC Output for each of : VIO_B_M2C,
...
//                      Aug 31, 2011: 2.01 Fix issue related to write
//                      timeout
//                                         when code running in BRAM
//                      Feb 20, 2012: 2.02 Include "xbasic_types.h"
//                      for Xuint data types
//
//----------------------------------------------------------------
...
int fmc_ipmi_get_common_info(fmc_iic_t *pIIC,
                             Xuint8 iic_address,
  ...
  for (idx = 0; idx < 8; idx++) {
    num_bytes = pIIC->fpIicRead(pIIC, (Xint8)(iic_address >> 1), idx,
                                &data[idx], 1);
    if (num_bytes != 1) {
      // I2C error, abort
      xil_printf(
          "[fmc_ipmi_get_common_info] FRU_I2C_ERROR (idx=%d)\n\r",
          idx);
...
int fmc_ipmi_get_board_info(fmc_iic_t *pIIC,
                            Xuint8 iic_address,
  ...
  for (idx = 0; idx < board_info->area_len; idx++) {
    num_bytes =
        pIIC->fpIicRead(pIIC, (Xint8)(iic_address >> 1), offset + idx,
                        &board_info->area_data[idx], 1);
    if (num_bytes != 1) {
      // I2C error, abort
      xil_printf(
          "[fmc_ipmi_get_board_info] FRU_I2C_ERROR (idx=%d)\n\r",
          idx);
      return FRU_I2C_ERROR;
    }
  }
...
int fmc_ipmi_set_common_info(fmc_iic_t *pIIC,
                             Xuint8 iic_address,
                             struct fru_header *common_info) {
  Xuint8 num_bytes;
  Xuint8 data[8];
  ...
  for (idx = 0; idx < 8; idx++) {
    num_bytes = pIIC->fpIicWrite(pIIC, (Xint8)(iic_address >> 1), idx,
                                 &data[idx], 1);
    if (num_bytes != 1) {
      // I2C error, abort
      xil_printf(
          "[fmc_ipmi_set_common_info] FRU_I2C_ERROR (idx=%d)\n\r",
          idx);
      return FRU_I2C_ERROR;
    }
    ...
    do {
      ...
      if (timeout > 100) {
        // I2C error, abort
        xil_printf(
            "[fmc_ipmi_set_common_info] FRU_I2C_TIMEOUT (idx=%d)\n\r",
            idx);
        return FRU_I2C_TIMEOUT;
      }

      num_bytes = pIIC->fpIicRead(pIIC, (Xint8)(iic_address >> 1),
                                  idx, &data[idx], 1);
    } while (!num_bytes);
  }
...
int fmc_ipmi_set_board_info(fmc_iic_t *pIIC,
                            Xuint8 iic_address,
                            struct fru_area_board *board_info) {
  Xuint8 num_bytes;
  Xuint8 data;
  ...
  // Area length (multiples of 8 bytes)
  board_info->area_data[idx++] =
      (Xuint8)((board_info->area_len) >> 3);
  // Language
  board_info->area_data[idx++] = board_info->lang;
  // Mrg. Data / Time - (little endian)
  board_info->area_data[idx++] =
      (Xuint8)(board_info->mfg_date_time >> 0 & 0xFF);
  board_info->area_data[idx++] =
      (Xuint8)(board_info->mfg_date_time >> 8 & 0xFF);
  board_info->area_data[idx++] =
      (Xuint8)(board_info->mfg_date_time >> 16 & 0xFF);
  // Board Manufacturer (8 bit ASCII)
  board_info->area_data[idx++] =
      0xC0 | (strlen(board_info->mfg) + 1); // type / length byte
  memcpy(&board_info->area_data[idx], board_info->mfg,
         strlen(board_info->mfg) + 1); // data bytes
  idx += strlen(board_info->mfg) + 1;
  // Board Product Name (8 bit ASCII)
  board_info->area_data[idx++] =
      0xC0 | (strlen(board_info->prod) + 1); // type / length byte
  memcpy(&board_info->area_data[idx], board_info->prod,
         strlen(board_info->prod) + 1); // data bytes
  idx += strlen(board_info->prod) + 1;
  // Board Serial Number (8 bit ASCII)
  board_info->area_data[idx++] =
      0xC0 | (strlen(board_info->serial) + 1); // type / length byte
  memcpy(&board_info->area_data[idx], board_info->serial,
         strlen(board_info->serial) + 1); // data bytes
  idx += strlen(board_info->serial) + 1;
  // Board Part Number (8 bit ASCII)
  board_info->area_data[idx++] =
      0xC0 | (strlen(board_info->part) + 1); // type / length byte
  memcpy(&board_info->area_data[idx], board_info->part,
         strlen(board_info->part) + 1); // data bytes
  idx += strlen(board_info->part) + 1;
  // FRU File (8 bit ASCII)
  board_info->area_data[idx++] =
      0xC0 | (strlen(board_info->fru) + 1); // type / length byte
  memcpy(&board_info->area_data[idx], board_info->fru,
         strlen(board_info->fru) + 1); // data bytes
  ...
  // Write area to EEPROM via I2C
  for (idx = 0; idx < board_info->area_len; idx++) {
    num_bytes = pIIC->fpIicWrite(pIIC, (Xint8)(iic_address >> 1),
                                 offset + idx,
                                 &board_info->area_data[idx], 1);
    if (num_bytes != 1) {
      // I2C error, abort
      xil_printf(
          "[fmc_ipmi_set_board_info] FRU_I2C_ERROR (idx=%d)\n\r",
          idx);
      return FRU_I2C_ERROR;
    }
    ...
    do {
      ...
      if (timeout > 100) {
        // I2C error, abort
        xil_printf(
            "[fmc_ipmi_set_board_info] FRU_I2C_TIMEOUT (idx=%d)\n\r",
            idx);
        return FRU_I2C_TIMEOUT;
      }
