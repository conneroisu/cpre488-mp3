...
//
// Disclaimer:
//    Avnet, Inc. makes no warranty for the use of this code or
//    design. This code is provided  "As Is". Avnet, Inc assumes no
//    responsibility for any errors, which may appear in this code,
//    nor does it make a commitment to update the information
//    contained herein. Avnet, Inc specifically disclaims any implied
...
//                      Initial version generated with EDK Create
//                      Peripheral Wizard
//                      - contained generic macros to access 32
//                      registers Driver modified to add:
...
//                                         - port to Kintex-7
//                      Feb 09, 2011: 1.08 Add VITA configuration
//                      functions
//                                         - init
//                                         - get_status
//                                         - set_exposure
//                                         - set_analog_gain
//                                         - set_digital_gain
//                      Jun 01, 2012: 1.12 Change syncgen
//                      configuration code Dec 17, 2013: 2.01 Port to
//                      Vivado 2013.3
//----------------------------------------------------------------
...
//                                         - Update init sequence to
//                                         resolve intermittent issues
//                                            - Reset the camera
//                                            receiver before starting
//                                            the sensor
//                                            - Move start of capture
//                                            to new
//                                            SENSOR_INIT_STREAMON
//                                            sequence
...
Xuint16 vita_grayimage_on1_seq[VITA_GRAYIMAGE_ON_QTY][3] = {
    ...
    {219, 0xFFFF, 0x3E2D}, {220, 0xFFFF, 0x674F},
    {429, 0xFFFF, 0x0100}, {430, 0xFFFF, 0x1BF1},
    {431, 0xFFFF, 0x1BC3}, {432, 0xFFFF, 0x1BC2},
    {435, 0xFFFF, 0x2142}, {436, 0xFFFF, 0x2142},
    {463, 0xFFFF, 0x0100}, {464, 0xFFFF, 0x0FE4},
    {465, 0xFFFF, 0x0BC2}, {472, 0xFFFF, 0x0B46},
    {475, 0xFFFF, 0x2142}, {476, 0xFFFF, 0x2142}};
Xuint16 vita_grayimage_on2_seq[VITA_GRAYIMAGE_ON_QTY][3] = {
    // Gray Image ON (for vita_refclk in 45-55MHz range)
    {219, 0xFFFF, 0x3E2D}, {220, 0xFFFF, 0x674F},
    {429, 0xFFFF, 0x0100}, {430, 0xFFFF, 0x1BF1},
    {431, 0xFFFF, 0x1BC3}, {432, 0xFFFF, 0x1BC2},
    {435, 0xFFFF, 0x2141}, {436, 0xFFFF, 0x2142},
    {463, 0xFFFF, 0x0100}, {464, 0xFFFF, 0x0FE4},
    {465, 0xFFFF, 0x0BC2}, {472, 0xFFFF, 0x0B46},
    {475, 0xFFFF, 0x2141}, {476, 0xFFFF, 0x2141}};
Xuint16 vita_grayimage_on3_seq[VITA_GRAYIMAGE_ON_QTY][3] = {
    // Gray Image ON (for vita_refclk in 30-45MHz range)
    {219, 0xFFFF, 0x3E2D}, {220, 0xFFFF, 0x674F},
    {429, 0xFFFF, 0x0100}, {430, 0xFFFF, 0x1BF1},
    {431, 0xFFFF, 0x1BC3}, {432, 0xFFFF, 0x1BC2},
    {435, 0xFFFF, 0x2141}, {436, 0xFFFF, 0x2141},
    {463, 0xFFFF, 0x0100}, {464, 0xFFFF, 0x0FE4},
    {465, 0xFFFF, 0x0BC2}, {472, 0xFFFF, 0x0B46},
    {475, 0xFFFF, 0x2141}, {476, 0xFFFF, 0x2141}};
...
Xuint16 vita_grayimage_off_seq[VITA_GRAYIMAGE_OFF_QTY][3] = {
    // Gray Image OFF
    {219, 0xFFFF, 0x3E2E}, {220, 0xFFFF, 0x6750},
    {429, 0xFFFF, 0x7150}, {430, 0xFFFF, 0x0100},
    {431, 0xFFFF, 0x03F1}, {432, 0xFFFF, 0x03C5},
    {435, 0xFFFF, 0x214F}, {436, 0xFFFF, 0x2145},
    {463, 0xFFFF, 0x0000}, {464, 0xFFFF, 0x0100},
    {465, 0xFFFF, 0x0BE6}, {472, 0xFFFF, 0x1346},
    {475, 0xFFFF, 0x214F}, {476, 0xFFFF, 0x2145}};
...
Xuint16 vita_mult_timer_line_resolution_seq
    [VITA_MULT_TIMER_LINE_RESOLUTION_QTY][3] = {
        // R199[15:0] mult_timer = (1920+88+44+148)/4 = 2200/4 = 550
        // (0x0226)
        // 199, 0xFFFF, 0x0226
        // R199[15:0] mult_timer = (1920+88+44+132)/4 = 2184/4 = 546
        // (0x0222)
        {199, 0xFFFF, 0x0222}};
...
Xuint16 vita_spi_again_values[VITA_SPI_AGAIN_QTY][3] = {
    {204, 0xFFFF,
     0x01E2}, // 1.00 : gain_state1=0x02(1.00), gain_stage2=0xF(1.00)
    {204, 0xFFFF,
     0x00E2}, // 1.14 : gain_state1=0x02(1.00), gain_stage2=0x7(1.14)
    {204, 0xFFFF,
     0x0062}, // 1.33 : gain_state1=0x02(1.00), gain_stage2=0x3(1.33)
    {204, 0xFFFF,
     0x00A2}, // 1.60 : gain_state1=0x02(1.00), gain_stage2=0x5(1.60)
    {204, 0xFFFF,
     0x0022}, // 2.00 : gain_state1=0x02(1.00), gain_stage2=0x1(2.00)
    {204, 0xFFFF,
     0x00E1}, // 2.29 : gain_state1=0x01(2.00), gain_stage2=0x7(1.14)
    {204, 0xFFFF,
     0x0061}, // 2.67 : gain_state1=0x01(2.00), gain_stage2=0x3(1.33)
    {204, 0xFFFF,
     0x00A1}, // 3.20 : gain_state1=0x01(2.00), gain_stage2=0x5(1.60)
    {204, 0xFFFF,
     0x0021}, // 4.00 : gain_state1=0x01(2.00), gain_stage2=0x1(2.00)
    {204, 0xFFFF,
     0x00C1}, // 5.33 : gain_state1=0x01(2.00), gain_stage2=0x6(2.67)
    {204, 0xFFFF,
     0x0041} // 8.00 : gain_state1=0x01(2.00), gain_stage2=0x2(4.00)
};
 ...
 *
 * @param    pContext contains a pointer to the new VITA instance's
 *context.
 * @param    szName contains a string describing the new VITA
 *instance.
 * @param    uBaseAddr_SPI contains the base address of the
 *ONSEMI_VITA_SPI pcore.
 * @param    uBaseAddr_CAM contains the base address of the
 *ONSEMI_VITA_CAM pcore.
 *
 ...
 *
 ******************************************************************************/
int onsemi_vita_init(onsemi_vita_t *pContext,
                     char szName[],
  ...
  uCoreVersion = onsemi_vita_spi_reg_read(
      pContext, ONSEMI_VITA_SPI_CORE_VERSION_REG);
  uCoreID =
      onsemi_vita_spi_reg_read(pContext, ONSEMI_VITA_SPI_CORE_ID_REG);
  if (uCoreID == ONSEMI_VITA_SPI_CORE_ID_VAL) {
    if (((uCoreVersion & 0xFF000000) >> 24) ==
        ((ONSEMI_VITA_SPI_CORE_VERSION_VAL & 0xFF000000) >> 24)) {
      // xil_printf( "[onsemi_vita_init] Detected Compatible Version
      // %d.%d.%d of ONSEMI_VITA_SPI core\n\r",
      // ((uCoreVersion&0xFF000000)>>24),
      // ((uCoreVersion&0x00FF0000)>>16), (uCoreVersion&0x0000FFFF) );
    } else {
      // xil_printf( "[onsemi_vita_init] Detected Incompatible Version
      // %d.%d.%d of ONSEMI_VITA_SPI core\n\r",
      // ((uCoreVersion&0xFF000000)>>24),
      // ((uCoreVersion&0x00FF0000)>>16), (uCoreVersion&0x0000FFFF) );
      return 0;
    ...
    // xil_printf( "[onsemi_vita_init] Failed to detect
    // ONSEMI_VITA_SPI core\n\r" );
    return 0;
  }

  // Detection of onsemi_vita_cam core
  uCoreVersion = onsemi_vita_cam_reg_read(
      pContext, ONSEMI_VITA_CAM_CORE_VERSION_REG);
  uCoreID =
      onsemi_vita_cam_reg_read(pContext, ONSEMI_VITA_CAM_CORE_ID_REG);
  if (uCoreID == ONSEMI_VITA_CAM_CORE_ID_VAL) {
    if (((uCoreVersion & 0xFF000000) >> 24) ==
        ((ONSEMI_VITA_CAM_CORE_VERSION_VAL & 0xFF000000) >> 24)) {
      // xil_printf( "[onsemi_vita_init] Detected Compatible Version
      // %d.%d.%d of ONSEMI_VITA_CAM core\n\r",
      // ((uCoreVersion&0xFF000000)>>24),
      // ((uCoreVersion&0x00FF0000)>>16), (uCoreVersion&0x0000FFFF) );
    } else {
      // xil_printf( "[onsemi_vita_init] Detected Incompatible Version
 ...
 *
 ******************************************************************************/
Xuint32 onsemi_vita_spi_reg_read(onsemi_vita_t *pContext,
                                 Xuint32 uRegOffset) {
  Xuint32 uData;

  uData = ONSEMI_VITA_SPI_mReadSlaveReg0(pContext->uBaseAddr_SPI,
                                         uRegOffset);

  return uData;
}
Xuint32 onsemi_vita_cam_reg_read(onsemi_vita_t *pContext,
                                 Xuint32 uRegOffset) {
  Xuint32 uData;

  uData = ONSEMI_VITA_CAM_mReadSlaveReg0(pContext->uBaseAddr_CAM,
                                         uRegOffset);
 ...
 *
 ******************************************************************************/
void onsemi_vita_spi_reg_write(onsemi_vita_t *pContext,
                               Xuint32 uRegOffset,
                               Xuint32 uData) {
  ONSEMI_VITA_SPI_mWriteSlaveReg0(pContext->uBaseAddr_SPI, uRegOffset,
                                  uData);
}
void onsemi_vita_cam_reg_write(onsemi_vita_t *pContext,
                               Xuint32 uRegOffset,
                               Xuint32 uData) {
  ONSEMI_VITA_CAM_mWriteSlaveReg0(pContext->uBaseAddr_CAM, uRegOffset,
                                  uData);
}
...
int onsemi_vita_reset(onsemi_vita_t *pContext, Xuint32 uReset) {
  // ONSEMI_VITA_CAM_mWriteSlaveReg3(pContext->uBaseAddr_CAM, 0,
  // uReset);
  onsemi_vita_cam_reg_write(pContext, ONSEMI_VITA_CAM_CONTROL_REG,
                            uReset);
 ...
 * @param    pContext contains a pointer to the new VITA instance's
 *context.
 * @param    uTiming contains the 16 bit value to adjust the SPI
 *timing.
 *
 * @return   If successfull, returns 1.  Otherwise, returns 0.
 *
...
int onsemi_vita_spi_config(onsemi_vita_t *pContext, Xuint16 uTiming) {
  // ONSEMI_VITA_SPI_mWriteSlaveReg5(pContext->uBaseAddr_SPI, 0,
  // (Xuint32)uTiming);
  onsemi_vita_spi_reg_write(pContext, ONSEMI_VITA_SPI_TIMING_REG,
                            (Xuint32)uTiming);
 ...
 *
 ******************************************************************************/
int onsemi_vita_spi_read(onsemi_vita_t *pContext,
                         Xuint16 uAddr,
                         Xuint16 *pData) {
  Xuint32 uRequest;
  Xuint32 uResponse;
  ...
  uStatus =
      onsemi_vita_spi_reg_read(pContext, ONSEMI_VITA_SPI_STATUS_REG);
  ...
  while (!(uStatus & ONSEMI_VITA_SPI_RXFIFO_EMPTY_BIT)) {
    ...
    onsemi_vita_spi_reg_write(pContext, ONSEMI_VITA_SPI_RXFIFO_REG,
                              0xABBAABBA);

    uStatus = onsemi_vita_spi_reg_read(pContext,
                                       ONSEMI_VITA_SPI_STATUS_REG);
    // xil_printf( "[onsemi_vita_spi_read ] Status   = 0x%08X\n\r",
    // uStatus );
  }
  ...
  do {
    ...
    // xil_printf( "[onsemi_vita_spi_read ] Status   = 0x%08X\n\r",
    // uStatus );
  } while ((uStatus & ONSEMI_VITA_SPI_TXFIFO_FULL_BIT) &&
           (--timeout));

  // Send Request
  uRequest =
      (ONSEMI_VITA_SPI_READ_BIT) | (((Xuint32)uAddr) << 16) | 0x0000;
  // xil_printf( "[onsemi_vita_spi_read ] Request  = 0x%08X\n\r",
  // uRequest );
  onsemi_vita_spi_reg_write(pContext, ONSEMI_VITA_SPI_TXFIFO_REG,
                            uRequest);

  if (!timeout) {
    xil_printf("[onsemi_vita_spi_read ] Timed out waiting for "
               "!TXFIFO_FULL\n\r");
    return 0;
  }
  ...
  // Get Response
  uResponse =
      onsemi_vita_spi_reg_read(pContext, ONSEMI_VITA_SPI_RXFIFO_REG);
 ...
 *
 ******************************************************************************/
int onsemi_vita_spi_write(onsemi_vita_t *pContext,
                          Xuint16 uAddr,
                          Xuint16 uData) {
  Xuint32 uRequest;
  Xuint32 uResponse;
  ...
  if (!timeout) {
    xil_printf("[onsemi_vita_spi_write] Timed out waiting for "
               "!TXFIFO_FULL\n\r");
    return 0;
  }

  // Send Request
  uRequest = (ONSEMI_VITA_SPI_WRITE_BIT) | (((Xuint32)uAddr) << 16) |
             ((Xuint16)uData);
...
int onsemi_vita_spi_nop(onsemi_vita_t *pContext) {
  ...
  if (!timeout) {
    xil_printf("[onsemi_vita_spi_nop  ] Timed out waiting for "
               "!TXFIFO_FULL\n\r");
    return 0;
  }
 ...
 * @param    uLength contains the number of address/mask/value sets in
 *sequence.
 *
 * @return   If successfull, returns 1.  Otherwise, returns 0.
 *
 ...
 *
 ******************************************************************************/
int onsemi_vita_spi_write_sequence(onsemi_vita_t *pContext,
                                   Xuint16 pConfig[][3],
                                   Xuint32 uLength) {
  Xuint16 uData;
  int i;
  ...
  for (i = 0; i < uLength; i++) {
    ...
    // Insert NOPs between transactions to respect following
    // requirement:
 ...
 *
 ******************************************************************************/
int onsemi_vita_spi_display_sequence(onsemi_vita_t *pContext,
                                     Xuint16 pConfig[][3],
                                     Xuint32 uLength) {
  Xuint16 uData;
  int i;
 ...
 * @param    pContext contains a pointer to the new VITA instance's
 *context.
 * @param    initId identifies which portion of the initialization is
 *to be performed. The initialization sequences correspond to the VITA
 *datasheet
 *
 *              - SENSOR_INIT_SEQ00 => Assert/Deassert RESET_N pin
 *              - SENSOR_INIT_SEQ01 => Enable Clock Management - Part
 *1
 *              - SENSOR_INIT_SEQ02 => Verify PLL Lock Indicator
 *              - SENSOR_INIT_SEQ03 => Enable Clock Management - Part
 *2
 ...
 *              - SENSOR_INIT_SEQ09 => Disable Clock Management - Part
 *2
 *              - SENSOR_INIT_SEQ10 => Disable Clock Management - Part
 *1
 *
 *              - SENSOR_INIT_ENABLE  => perform SEQ00 to SEQ06
 *              - SENSOR_INIT_DISABLE => perform SEQ06 to SEQ10
 *
 * @param    bVerbose identified wether or not to display verbose
 *information.
 *
 * @return   If successfull, returns 1.  Otherwise, returns 0.
 *
 ...
 *
 ******************************************************************************/
int onsemi_vita_sensor_initialize(onsemi_vita_t *pContext,
                                  int initID,
                                  int bVerbose) {
  int i;
  Xuint16 uAddr;
  ...
  if ((initID == SENSOR_INIT_SEQ00) ||
      (initID == SENSOR_INIT_ENABLE)) {
    // Configure Sync Generator
    {
      ...
      //{ 1080,    4,    5,   36,    1, 1920,   88,   44,  148,    1
      ...
      if (bVerbose) {
        ...
        xil_printf("\tHSYNC Timing     = hav=%04d, hfp,=%02d, "
                   "hsw=%02d (hsp=%d), hbp=%03d (x4)\n\r",
                   h_active, h_fporch, h_syncwidth, h_syncpol,
                   h_bporch);
        xil_printf("\tVSYNC Timing     = hav=%04d, hfp,=%02d, "
                   "hsw=%02d (hsp=%d), hbp=%03d (x4)\n\r",
                   v_active, v_fporch, v_syncwidth, v_syncpol,
                   v_bporch);
      }

      // Horizontal settings
      onsemi_vita_cam_reg_write(pContext,
                                ONSEMI_VITA_CAM_SYNCGEN_DELAY_REG,
                                ((1920 + 88 + 44 + 148) >> 2) *
                                    6); // approx. 6 lines of delay
      onsemi_vita_cam_reg_write(pContext,
                                ONSEMI_VITA_CAM_SYNCGEN_HTIMING1_REG,
                                (h_active) << 0 | (h_fporch) << 16);
      onsemi_vita_cam_reg_write(
          pContext, ONSEMI_VITA_CAM_SYNCGEN_HTIMING2_REG,
          (h_syncpol) << 15 | (h_syncwidth) << 0 | (h_bporch) << 16);
      // Vertical settings
      onsemi_vita_cam_reg_write(pContext,
                                ONSEMI_VITA_CAM_SYNCGEN_VTIMING1_REG,
                                (v_active) << 0 | (v_fporch) << 16);
      onsemi_vita_cam_reg_write(
          pContext, ONSEMI_VITA_CAM_SYNCGEN_VTIMING2_REG,
          (v_syncpol) << 15 | (v_syncwidth) << 0 | (v_bporch) << 16);
    }

    if (bVerbose)
      xil_printf("VITA ISERDES - Setting Training Sequence to "
                 "0x000003A6\n\r");
    onsemi_vita_cam_reg_write(
        pContext, ONSEMI_VITA_CAM_ISERDES_TRAINING_REG, 0x000003A6);

    if (bVerbose)
      xil_printf("VITA ISERDES - Setting Manual Tap to 0x%08X\n\r",
                 pContext->uManualTap);
    onsemi_vita_cam_reg_write(pContext,
                              ONSEMI_VITA_CAM_ISERDES_MANUAL_TAP_REG,
                              pContext->uManualTap);

    if (bVerbose)
      xil_printf("VITA DECODER - Configuring Sync Codes\n\r");
    onsemi_vita_cam_reg_write(
        pContext, ONSEMI_VITA_CAM_DECODER_STARTODDEVEN_REG,
        0x00000000);
    onsemi_vita_cam_reg_write(pContext,
                              ONSEMI_VITA_CAM_DECODER_CODES_LS_LE_REG,
                              (0x00AA) | (0x012A << 16));
    onsemi_vita_cam_reg_write(pContext,
                              ONSEMI_VITA_CAM_DECODER_CODES_FS_FE_REG,
                              (0x02AA) | (0x032A << 16));
    onsemi_vita_cam_reg_write(
        pContext, ONSEMI_VITA_CAM_DECODER_CODES_BL_IMG_REG,
        (0x0015) | (0x0035 << 16));
    onsemi_vita_cam_reg_write(
        pContext, ONSEMI_VITA_CAM_DECODER_CODES_TR_CRC_REG,
        (0x03A6) | (0x0059 << 16));

    if (bVerbose)
      xil_printf("VITA REMAPPER - Configuring for image lines in "
                 "normal mode\n\r");
    onsemi_vita_cam_reg_write(
        pContext, ONSEMI_VITA_CAM_REMAPPER_CONTROL_REG, 0x00000001);
    uControl = onsemi_vita_cam_reg_read(
        pContext, ONSEMI_VITA_CAM_REMAPPER_CONTROL_REG);
    if (bVerbose)
    ...
    onsemi_vita_cam_reg_write(pContext,
                              ONSEMI_VITA_CAM_ISERDES_CONTROL_REG,
                              ONSEMI_VITA_CAM_ISERDES_RESET_BIT);
    if (bVerbose)
      xil_printf("VITA DECODER - Asserting Reset\n\r");
    onsemi_vita_cam_reg_write(pContext,
                              ONSEMI_VITA_CAM_DECODER_CONTROL_REG,
                              ONSEMI_VITA_CAM_DECODER_RESET_BIT);
    if (bVerbose)
      xil_printf("VITA CRC - Asserting Reset\n\r");
    onsemi_vita_cam_reg_write(pContext,
                              ONSEMI_VITA_CAM_CRC_CONTROL_REG,
                              ONSEMI_VITA_CAM_CRC_INITVALUE_BIT |
                                  ONSEMI_VITA_CAM_CRC_RESET_BIT);
    ...
    onsemi_vita_cam_reg_write(
        pContext, ONSEMI_VITA_CAM_ISERDES_CONTROL_REG, 0x00000000);
    if (bVerbose)
      xil_printf("VITA DECODER - Releasing Reset\n\r");
    onsemi_vita_cam_reg_write(
        pContext, ONSEMI_VITA_CAM_DECODER_CONTROL_REG, 0x00000000);
    if (bVerbose)
    ...
    if (uData != 0x5614) {
      if (bVerbose)
        xil_printf(
            "\tERROR: Absent or unsupported VITA sensor !!!\n\r");
      return 0;
    }
  }

  if ((initID == SENSOR_INIT_SEQ01) ||
      (initID == SENSOR_INIT_ENABLE)) {
    if (bVerbose) {
      xil_printf("VITA SPI Sequence 1 - Enable Clock Management - "
                 "Part 1\n\r");
      onsemi_vita_spi_display_sequence(pContext, vita_spi_seq1,
                                       VITA_SPI_SEQ1_QTY);
    }
    onsemi_vita_spi_write_sequence(pContext, vita_spi_seq1,
                                   VITA_SPI_SEQ1_QTY);
  }

  if ((initID == SENSOR_INIT_SEQ02) ||
      (initID == SENSOR_INIT_ENABLE)) {
    Xuint16 uLock = 0;
    ...
    timeout = 10;
    while (!(uLock) && --timeout) {
      usleep(100000); // 100 ms  // phjones updated from millisleep to
                      // usleep
    ...
    if (!timeout) {
      if (bVerbose)
        xil_printf("\tERROR: Timed Out while waiting for PLL lock to "
                   "assert !!!\n\r");
      return 0;
    }
  }

  if ((initID == SENSOR_INIT_SEQ03) ||
      (initID == SENSOR_INIT_ENABLE)) {
    if (bVerbose) {
      xil_printf("VITA SPI Sequence 3 - Enable Clock Management - "
                 "Part 2\n\r");
      onsemi_vita_spi_display_sequence(pContext, vita_spi_seq3,
                                       VITA_SPI_SEQ3_QTY);
    }
    onsemi_vita_spi_write_sequence(pContext, vita_spi_seq3,
                                   VITA_SPI_SEQ3_QTY);
  }

  if ((initID == SENSOR_INIT_SEQ04) ||
      (initID == SENSOR_INIT_ENABLE)) {
    if (bVerbose) {
      xil_printf(
          "VITA SPI Sequence 4 - Required Register Upload\n\r");
      onsemi_vita_spi_display_sequence(pContext, vita_spi_seq4,
                                       VITA_SPI_SEQ4_QTY);
    }
    onsemi_vita_spi_write_sequence(pContext, vita_spi_seq4,
                                   VITA_SPI_SEQ4_QTY);
  }

  if ((initID == SENSOR_INIT_SEQ05) ||
      (initID == SENSOR_INIT_ENABLE)) {
    if (bVerbose) {
      xil_printf("VITA SPI Sequence 5 - Soft Power-Up\n\r");
      onsemi_vita_spi_display_sequence(pContext, vita_spi_seq5,
                                       VITA_SPI_SEQ5_QTY);
    }
    onsemi_vita_spi_write_sequence(pContext, vita_spi_seq5,
                                   VITA_SPI_SEQ5_QTY);
    //
  }

  if ((initID == SENSOR_INIT_SEQ06) ||
      (initID == SENSOR_INIT_ENABLE)) {
    if (bVerbose)
      xil_printf("VITA ISERDES - Asserting Reset\n\r");
    onsemi_vita_cam_reg_write(pContext,
    ...
    uStatus = onsemi_vita_cam_reg_read(
        pContext, ONSEMI_VITA_CAM_ISERDES_STATUS_REG);
    if (bVerbose)
    ...
    onsemi_vita_cam_reg_write(
        pContext, ONSEMI_VITA_CAM_ISERDES_CONTROL_REG,
        ONSEMI_VITA_CAM_ISERDES_ALIGN_START_BIT);
    ...
    onsemi_vita_cam_reg_write(
        pContext, ONSEMI_VITA_CAM_ISERDES_CONTROL_REG,
        ONSEMI_VITA_CAM_ISERDES_FIFO_ENABLE_BIT);
    if (bVerbose)
      xil_printf("VITA DECODER - Enabling Sync Channel Decoder\n\r");
    onsemi_vita_cam_reg_write(pContext,
                              ONSEMI_VITA_CAM_DECODER_CONTROL_REG,
                              ONSEMI_VITA_CAM_DECODER_ENABLE_BIT);
    uControl = onsemi_vita_cam_reg_read(
        pContext, ONSEMI_VITA_CAM_DECODER_CONTROL_REG);
    if (bVerbose)
    ...
    do {
      uStatus = onsemi_vita_cam_reg_read(
          pContext, ONSEMI_VITA_CAM_CRC_STATUS_REG);
      if (bVerbose)
  ...
  if ((initID == SENSOR_INIT_SEQ06A) ||
      (initID == SENSOR_INIT_STREAMON)) {
    if (bVerbose) {
      xil_printf("VITA SPI Sequence 6 - Enable Sequencer\n\r");
      onsemi_vita_spi_display_sequence(pContext, vita_spi_seq6,
                                       VITA_SPI_SEQ6_QTY);
    }
    onsemi_vita_spi_write_sequence(pContext, vita_spi_seq6,
                                   VITA_SPI_SEQ6_QTY);
  }

  if ((initID == SENSOR_INIT_SEQ07) ||
      (initID == SENSOR_INIT_DISABLE)) {
    if (bVerbose) {
      xil_printf("VITA SPI Sequence 7 - Disable Sequencer\n\r");
      onsemi_vita_spi_display_sequence(pContext, vita_spi_seq7,
                                       VITA_SPI_SEQ7_QTY);
    }
    onsemi_vita_spi_write_sequence(pContext, vita_spi_seq7,
                                   VITA_SPI_SEQ7_QTY);
  }

  if ((initID == SENSOR_INIT_SEQ08) ||
      (initID == SENSOR_INIT_DISABLE)) {
    if (bVerbose) {
      xil_printf("VITA SPI Sequence 8 - Soft Power-Down\n\r");
      onsemi_vita_spi_display_sequence(pContext, vita_spi_seq8,
                                       VITA_SPI_SEQ8_QTY);
    }
    onsemi_vita_spi_write_sequence(pContext, vita_spi_seq8,
                                   VITA_SPI_SEQ8_QTY);
  }

  if ((initID == SENSOR_INIT_SEQ09) ||
      (initID == SENSOR_INIT_DISABLE)) {
    if (bVerbose) {
      xil_printf("VITA SPI Sequence 9 - Disable Clock Management - "
                 "Part 2\n\r");
      onsemi_vita_spi_display_sequence(pContext, vita_spi_seq9,
                                       VITA_SPI_SEQ9_QTY);
    }
    onsemi_vita_spi_write_sequence(pContext, vita_spi_seq9,
                                   VITA_SPI_SEQ9_QTY);
  }

  if ((initID == SENSOR_INIT_SEQ10) ||
      (initID == SENSOR_INIT_DISABLE)) {
    if (bVerbose) {
      xil_printf("VITA SPI Sequence 10 - Disable Clock Management - "
                 "Part 1\n\r");
      onsemi_vita_spi_display_sequence(pContext, vita_spi_seqA,
                                       VITA_SPI_SEQA_QTY);
    }
    onsemi_vita_spi_write_sequence(pContext, vita_spi_seqA,
                                   VITA_SPI_SEQA_QTY);
  }
 ...
 *
 ******************************************************************************/
int onsemi_vita_get_status(onsemi_vita_t *pContext,
                           onsemi_vita_status_t *pStatus,
                           int bVerbose) {

  pStatus->cntBlackLines = onsemi_vita_cam_reg_read(
      pContext, ONSEMI_VITA_CAM_DECODER_CNT_BLACK_LINES_REG);
  pStatus->cntImageLines = onsemi_vita_cam_reg_read(
      pContext, ONSEMI_VITA_CAM_DECODER_CNT_IMAGE_LINES_REG);
  pStatus->cntBlackPixels = onsemi_vita_cam_reg_read(
      pContext, ONSEMI_VITA_CAM_DECODER_CNT_BLACK_PIXELS_REG);
  pStatus->cntImagePixels = onsemi_vita_cam_reg_read(
      pContext, ONSEMI_VITA_CAM_DECODER_CNT_IMAGE_PIXELS_REG);
  pStatus->cntFrames = onsemi_vita_cam_reg_read(
      pContext, ONSEMI_VITA_CAM_DECODER_CNT_FRAMES_REG);
  pStatus->cntWindows = onsemi_vita_cam_reg_read(
      pContext, ONSEMI_VITA_CAM_DECODER_CNT_WINDOWS_REG);
  pStatus->cntStartLines = onsemi_vita_cam_reg_read(
      pContext, ONSEMI_VITA_CAM_DECODER_CNT_START_LINES_REG);
  pStatus->cntEndLines = onsemi_vita_cam_reg_read(
      pContext, ONSEMI_VITA_CAM_DECODER_CNT_END_LINES_REG);
  pStatus->cntClocks = onsemi_vita_cam_reg_read(
      pContext, ONSEMI_VITA_CAM_DECODER_CNT_CLOCKS_REG);
  ...
  if (bVerbose) {
    ...
    xil_printf("\tBlack Pixels = %d\n\r",
               pStatus->cntBlackPixels * 4);
    xil_printf("\tImage Pixels = %d\n\r",
               pStatus->cntImagePixels * 4);
    xil_printf("\tFrames       = %d\n\r", pStatus->cntFrames);
    ...
    xil_printf("\t(End-Start)  = %d\n\r",
               pStatus->cntEndLines - pStatus->cntStartLines);

    xil_printf("\tClocks       = %d\n\r", pStatus->cntClocks);
  }

  pStatus->crcStatus = onsemi_vita_cam_reg_read(
      pContext, ONSEMI_VITA_CAM_CRC_STATUS_REG);
  if (bVerbose) {
    xil_printf("VITA CRC - Status = 0x%08X\n\r", pStatus->crcStatus);
  }
 ...
 *
 ******************************************************************************/
int onsemi_vita_sensor_1080P60(onsemi_vita_t *pContext,
                               int bVerbose) {
  Xuint16 vspi_data;
  ...
  // #   VREG-0x5C[15: 0] DELAY      = ((1920+88+44+128)/4) * 6 = 3300
  // (0x0CE4) vreg write 0x5C 0x00000CE4
  if (bVerbose)
    xil_printf("VITA 1080P60 - Tolerate 6 lines of jitter (required "
               "for programmable exposure)\n\r");
    ...
    xil_printf("VITA 1080P60 - Enable trig generator\n\r");
  {
    ...
    Xuint32 trigDutyCycle =
        90; // exposure time is 90% of frame time (ie. 15msec)
    vitaTrigGenDefaultFreq =
        (((1920 + 88 + 44 + 148) * (1080 + 4 + 5 + 36)) >> 2) - 2;

    onsemi_vita_cam_reg_write(
        pContext, ONSEMI_VITA_CAM_TRIGGEN_DEFAULT_FREQ_REG,
        vitaTrigGenDefaultFreq);
    if (bVerbose)
      xil_printf("\tVITA_REG[0x%04X] <= 0x%08X\n\r",
                 ONSEMI_VITA_CAM_TRIGGEN_DEFAULT_FREQ_REG,
                 vitaTrigGenDefaultFreq);
    // vitaTrigGenTrig0High   = (vitaTrigGenDefaultFreq *
    // trigDutyCycle)/100; // positive polarity
    vitaTrigGenTrig0High =
        (vitaTrigGenDefaultFreq * (100 - trigDutyCycle)) /
        100; // negative polarity

    vitaTrigGenTrig0Low = 1;
    onsemi_vita_cam_reg_write(pContext,
                              ONSEMI_VITA_CAM_TRIGGEN_TRIG0_HIGH_REG,
                              vitaTrigGenTrig0High);
    if (bVerbose)
      xil_printf("\tVITA_REG[0x%04X] <= 0x%08X\n\r",
                 ONSEMI_VITA_CAM_TRIGGEN_TRIG0_HIGH_REG,
                 vitaTrigGenTrig0High);
    onsemi_vita_cam_reg_write(pContext,
                              ONSEMI_VITA_CAM_TRIGGEN_TRIG0_LOW_REG,
                              vitaTrigGenTrig0Low);
    if (bVerbose)
      xil_printf("\tVITA_REG[0x%04X] <= 0x%08X\n\r",
                 ONSEMI_VITA_CAM_TRIGGEN_TRIG0_LOW_REG,
    ...
    vitaTrigGenControl =
        0x31000011; // invert trigger[2:0], internal trigger, enable
                    // trigger[0], update triggen_cnt registers
    onsemi_vita_cam_reg_write(pContext,
                              ONSEMI_VITA_CAM_TRIGGEN_CONTROL_REG,
                              vitaTrigGenControl);
    if (bVerbose)
      xil_printf("\tVITA_REG[0x%04X] <= 0x%08X\n\r",
                 ONSEMI_VITA_CAM_TRIGGEN_CONTROL_REG,
                 vitaTrigGenControl);
    vitaTrigGenControl = 0x30000011; // invert trigger[2:0], internal
                                     // trigger, enable trigger[0]
    onsemi_vita_cam_reg_write(pContext,
                              ONSEMI_VITA_CAM_TRIGGEN_CONTROL_REG,
                              vitaTrigGenControl);
    if (bVerbose)
 ...
 *              00 => 1.00 : gain_state1=0x02(1.00),
 *gain_stage2=0xF(1.00) 01 => 1.14 : gain_state1=0x02(1.00),
 *gain_stage2=0x7(1.14) 02 => 1.33 : gain_state1=0x02(1.00),
 *gain_stage2=0x3(1.33) 03 => 1.60 : gain_state1=0x02(1.00),
 *gain_stage2=0x5(1.60) 04 => 2.00 : gain_state1=0x02(1.00),
 *gain_stage2=0x1(2.00) 05 => 2.29 : gain_state1=0x01(2.00),
 *gain_stage2=0x7(1.14) 06 => 2.67 : gain_state1=0x01(2.00),
 *gain_stage2=0x3(1.33) 07 => 3.20 : gain_state1=0x01(2.00),
 *gain_stage2=0x5(1.60) 08 => 4.00 : gain_state1=0x01(2.00),
 *gain_stage2=0x1(2.00) 09 => 5.33 : gain_state1=0x01(2.00),
 *gain_stage2=0x6(2.67) 10 => 8.00 : gain_state1=0x01(2.00),
 *gain_stage2=0x2(4.00)
 * @param    bVerbose identified wether or not to display verbose
 *information.
 *
 * @return   If successfull, returns 1.  Otherwise, returns 0.
 ...
 *
 ******************************************************************************/
int onsemi_vita_set_analog_gain(onsemi_vita_t *pContext,
                                Xuint32 uAnalogGain,
                                int bVerbose) {
  int id;
  Xuint16 **seqData;
 ...
 *
 ******************************************************************************/
int onsemi_vita_set_digital_gain(onsemi_vita_t *pContext,
 ...
 * @param    pContext contains a pointer to the new VITA instance's
 *context.
 * @param    analogGain contains the value of the exposure time (in
 *usec)
 * @param    bVerbose identified wether or not to display verbose
 *information.
 *
 ...
 *
 ******************************************************************************/
int onsemi_vita_set_exposure_time(onsemi_vita_t *pContext,
                                  Xuint32 exposureTime,
                                  int bVerbose) {
  Xuint32 vitaTrigGenControl;
  Xuint32 vitaTrigGenDefaultFreq;
  Xuint32 vitaTrigGenTrig0High;
  ...
  vitaTrigGenDefaultFreq =
      (((1920 + 88 + 44 + 148) * (1080 + 4 + 5 + 36)) >> 2) - 2;
  if (bVerbose)
    xil_printf("\tTrigger = internal (%d fps, duty cycle = %d \%, "
               "period = %d cycles)...\r\n",
               trigFramesPerSec, trigDutyCycle,
               vitaTrigGenDefaultFreq + 2);
