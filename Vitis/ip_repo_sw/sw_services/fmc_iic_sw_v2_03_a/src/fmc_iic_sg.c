...
//                      Jan 12, 2010: 1.01 Add support for >256
//                      EEPROMs Jan 15, 2010:      Update to device
//                      specific pcores Jun 23, 2010: 1.02 Remove
//                      malloc from code Jul 20, 2010: 1.03 Add
//                      support for IicEWrite/IicERead Sep 03, 2010:
//                      Rename to fmc_iic_sg.c
...
#include "fmc_iic.h"
    ...
    defined(XPAR_SG_I2C_CONTROLLER_S6_PLBW_NUM_INSTANCES) ||         \
    defined(XPAR_SG_I2C_CONTROLLER_V6_PLBW_NUM_INSTANCES)
...
int fmc_iic_sg_IicWrite(fmc_iic_t *pIIC,
                        Xuint8 ChipAddress,
                        Xuint8 RegAddress,
                        Xuint8 *pBuffer,
                        Xuint8 ByteCount);
int fmc_iic_sg_IicRead(fmc_iic_t *pIIC,
                       Xuint8 ChipAddress,
                       Xuint8 RegAddress,
                       Xuint8 *pBuffer,
                       Xuint8 ByteCount);
int fmc_iic_sg_IicEWrite(fmc_iic_t *pIIC,
                         Xuint8 ChipAddress,
                         Xuint16 RegAddress,
                         Xuint8 *pBuffer,
                         Xuint8 ByteCount);
int fmc_iic_sg_IicERead(fmc_iic_t *pIIC,
                        Xuint8 ChipAddress,
                        Xuint16 RegAddress,
                        Xuint8 *pBuffer,
                        Xuint8 ByteCount);
 ...
 *
 ******************************************************************************/
int fmc_iic_sg_init(fmc_iic_t *pIIC,
                    char szName[],
                    void *pConfig,
                    Xuint32 delay) {
  ...
  // ); if ( pContext == NULL )
  //{
  //    xil_printf("Failed to allocate context data for FMC-IIC-XPS
  //    implementation\n\r" ); return 0;
  // }
  fmc_iic_sg_t *pContext = (fmc_iic_sg_t *)(pIIC->ContextBuffer);
  if (sizeof(fmc_iic_sg_t) > FMC_IIC_CONTEXT_BUFFER_SIZE) {
    xil_printf("FMC_IIC_CONTEXT_BUFFER_SIZE is not large enough for "
               "fmc_iic_sg_t structure (increase to %d)\n\r",
               sizeof(fmc_iic_sg_t));
    return 0;
  }
  ...
  // obtain the memory locations for storing the settings of shared
  // memories
  xc_get_shmem((pContext->iface0), "cmd_request",
               (void **)&(pContext->CmdRequest0));
  xc_get_shmem((pContext->iface0), "cmd_response",
               (void **)&(pContext->CmdResponse0));
  xc_get_shmem((pContext->iface0), "gpio_out8",
               (void **)&(pContext->GpioOut0));

  // init GPIO[7:0] to 0
  pContext->GpioOutData0 = 0x00;
  xc_write((pContext->iface0), pContext->GpioOut0->din,
           pContext->GpioOutData0);
...

  {
    ...
    // Send Request1
    do {
      xc_read((pContext->iface0), pContext->CmdRequest0->full,
              &status);
    } while (status);
    ...
    // Get Response1
    do {
      xc_read((pContext->iface0), pContext->CmdResponse0->empty,
              &status);
    } while (status);
    xc_read((pContext->iface0), pContext->CmdResponse0->dout,
            &cmd_rsp);
 ...
 * @param    RegAddress contains the address of the register to write
 *to.
 * @param    pBuffer contains the address of the data to write.
 * @param    ByteCount contains the number of bytes in the buffer to
 *be written. Note that this should not exceed the page size as noted
 *by the constant PAGE_SIZE.
 *
 * @return   The number of bytes written, a value less than that which
 *was specified as an input indicates an error.
 *
 * @note     None.
 *
 ******************************************************************************/
int fmc_iic_sg_IicWrite(fmc_iic_t *pIIC,
  ...
  for (SentByteCount = 0; SentByteCount < ByteCount;
       SentByteCount++) {
    // Format request
    cmd_req = (PROCESS_I2C_REG_WRITE << 24) |
              ((uint32_t)ChipAddress << 16 + 1) |
              ((uint32_t)RegAddress << 8) |
              (uint32_t)pBuffer[SentByteCount];
    // xil_printf( "[%s] fmc_iic_sg_write : cmd_req = 0x%08X\n\r",
    // pIIC->szName, cmd_req );
 ...
 * @param    RegAddress contains the address of the register to write
 *to.
 * @param    pBuffer contains the address of the data buffer to be
 *filled.
 * @param    ByteCount contains the number of bytes in the buffer to
 *be read. This value is constrained by the page size of the device
 *such that up to 64K may be read in one call.
 *
 * @return   The number of bytes read. A value less than the specified
 *input value indicates an error.
 *
 * @note     None.
 *
 ******************************************************************************/
int fmc_iic_sg_IicRead(fmc_iic_t *pIIC,
  ...
  for (ReceivedByteCount = 0; ReceivedByteCount < ByteCount;
       ReceivedByteCount++) {
    // Format request
    cmd_req = (PROCESS_I2C_REG_READ << 24) |
              ((uint32_t)ChipAddress << 16 + 1) |
              ((uint32_t)RegAddress << 8);
...
int fmc_iic_sg_IicEWrite(fmc_iic_t *pIIC,
  ...
  cmd_req = (PROCESS_I2C_WRITE_BYTE << 24) |
            ((uint32_t)ChipAddress << 16 + 1);
  // xil_printf( "[%s] fmc_iic_sg_write : cmd_req = 0x%08X\n\r",
  // pIIC->szName, cmd_req );
  ...
  cmd_req = (PROCESS_I2C_WRITE_BYTE << 24) |
            ((uint32_t)((RegAddress >> 8) & 0x00FF) << 16);
  // xil_printf( "[%s] fmc_iic_sg_write : cmd_req = 0x%08X\n\r",
  // pIIC->szName, cmd_req );
  ...
  for (SentByteCount = 0; SentByteCount < ByteCount;
    ...
    cmd_req = (PROCESS_I2C_WRITE_BYTE << 24) |
              ((uint32_t)pBuffer[SentByteCount] << 16);
    // xil_printf( "[%s] fmc_iic_sg_write : cmd_req = 0x%08X\n\r",
    // pIIC->szName, cmd_req );
...
int fmc_iic_sg_IicERead(fmc_iic_t *pIIC,
  ...
  cmd_req = (PROCESS_I2C_WRITE_BYTE << 24) |
            ((uint32_t)ChipAddress << 16 + 1) | 0x00010000;
  // xil_printf( "[%s] fmc_iic_sg_write : cmd_req = 0x%08X\n\r",
  // pIIC->szName, cmd_req );
  ...
  for (ReceivedByteCount = 0; ReceivedByteCount < ByteCount - 1;
       ReceivedByteCount++) {
    ...
    pBuffer[ReceivedByteCount] =
        (Xuint8)((cmd_rsp & 0x00FF0000) >> 16);
  }
...
#endif // defined(XPAR_SG_I2C_CONTROLLER_S3ADSP_PLBW_NUM_INSTANCES) ||
       // defined(XPAR_SG_I2C_CONTROLLER_S6_PLBW_NUM_INSTANCES) ||
       // defined(XPAR_SG_I2C_CONTROLLER_V6_PLBW_NUM_INSTANCES)
