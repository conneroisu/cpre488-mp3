...
//                      EEPROMs Jun 23, 2010: 1.02 Remove malloc from
//                      code
//                      ------------------------------------------
//                      Apr 12, 2011: 2.01 Update code to support
//                      iic_v2_02_a Aug 31, 2011: 2.02 Make sure all
//                      the Fifo's are cleared and Bus is Not busy Feb
//                      20, 2012: 2.03 Rename file "fmc_iic_axi.c" to
//                      "fmc_iic_axi.c"
//                                         Use "xil_io.h" instead of
//                                         "xio.h"
//
//----------------------------------------------------------------
 ...
 * The write function should be called with this as a maximum byte
 * count.
 */
#define PAGE_SIZE 2
...
int fmc_iic_axi_IicWrite(fmc_iic_t *pIIC,
                         Xuint8 ChipAddress,
                         Xuint8 RegAddress,
                         Xuint8 *pBuffer,
                         Xuint8 ByteCount);
int fmc_iic_axi_IicRead(fmc_iic_t *pIIC,
                        Xuint8 ChipAddress,
                        Xuint8 RegAddress,
                        Xuint8 *pBuffer,
                        Xuint8 ByteCount);
int fmc_iic_axi_IicEWrite(fmc_iic_t *pIIC,
                          Xuint8 ChipAddress,
                          Xuint16 RegAddress,
                          Xuint8 *pBuffer,
                          Xuint8 ByteCount);
int fmc_iic_axi_IicERead(fmc_iic_t *pIIC,
                         Xuint8 ChipAddress,
                         Xuint16 RegAddress,
                         Xuint8 *pBuffer,
                         Xuint8 ByteCount);
 ...
 *
 ******************************************************************************/
int fmc_iic_axi_init(fmc_iic_t *pIIC,
                     char szName[],
                     Xuint32 CoreAddress) {
  XStatus Status;
  Xuint8 StatusReg;
  ...
  // fmc_iic_axi_t *pContext = (fmc_iic_axi_t *)malloc(
  // sizeof(fmc_iic_axi_t) ); if ( pContext == NULL )
  //{
  //    xil_printf("Failed to allocate context data for FMC-IIC-AXI
  //    implementation\n\r" ); return 0;
  // }
  ...
  if (sizeof(fmc_iic_axi_t) > FMC_IIC_CONTEXT_BUFFER_SIZE) {
    xil_printf("FMC_IIC_CONTEXT_BUFFER_SIZE is not large enough for "
               "fic_iic_xps_t structure (increase to %d)\n\r",
               sizeof(fmc_iic_axi_t));
    return 0;
  }
  ...
  do {
    ...
    // xil_printf("[%s] Xil_In8(pContext->CoreAddress +
    // XIIC_SR_REG_OFFSET) => 0x%02X\n\r", pContext->szName, StatusReg
    // );
    StatusReg = StatusReg &
...
int fmc_iic_axi_GpoWrite(fmc_iic_t *pIIC, Xuint32 GpioData) {
  ...
  // xil_printf("[%s] Xil_Out32((pContext->CoreAddress + 0x124),data)
  // => 0x%08X\n\r", pContext->szName, Xil_In32(pContext->CoreAddress
  // + 0x124) );
...
int fmc_iic_axi_IicWrite(fmc_iic_t *pIIC,
   ...
   * address and the data to be written, put the address in first
   */
...
int fmc_iic_axi_IicRead(fmc_iic_t *pIIC,
  ...
  do {
    ...
    if (!(StatusReg & XIIC_SR_BUS_BUSY_MASK)) {
      SentByteCount =
          XIic_DynSend(pContext->CoreAddress, ChipAddress,
...
int fmc_iic_axi_IicEWrite(fmc_iic_t *pIIC,
   ...
   */
  SentByteCount = XIic_DynSend(pContext->CoreAddress, ChipAddress,
                               WriteBuffer, ByteCount + 2, XIIC_STOP);
  if (SentByteCount < 1) {
    SentByteCount = 1;
  }
...
int fmc_iic_axi_IicERead(fmc_iic_t *pIIC,
  ...
  do {
    ...
    if (!(StatusReg & XIIC_SR_BUS_BUSY_MASK)) {
      SentByteCount =
          XIic_DynSend(pContext->CoreAddress, ChipAddress,
                       WriteBuffer, 2, XIIC_REPEATED_START);
    }
    cnt++;
  } while (SentByteCount != 1 && (cnt < 100));
