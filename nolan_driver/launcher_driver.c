...
static void miss_launch_read_bulk_callback(struct urb *urb)
{
	struct usb_miss_launch *dev;
...
static int miss_launch_do_read_io(struct usb_miss_launch *dev, size_t count)
{
	int rv;
	...
	usb_fill_bulk_urb(dev->bulk_in_urb,
			...
			miss_launch_read_bulk_callback,
			dev);
...
retry:
	...
	if (dev->bulk_in_filled) {
		...
		if (!available) {
			 ...
			 */
			rv = miss_launch_do_read_io(dev, count);
			if (rv < 0)
				goto exit;
			else
				goto retry;
		 ...
		 */
		if (available < count)
			miss_launch_do_read_io(dev, count - chunk);
	} else {
...
static void miss_launch_write_bulk_callback(struct urb *urb)
{
	struct usb_miss_launch *dev;
			  ...
			  size_t count, loff_t *ppos)
{
	...
	usb_fill_bulk_urb(urb, dev->udev,
			  ...
			  buf, writesize, miss_launch_write_bulk_callback, dev);
