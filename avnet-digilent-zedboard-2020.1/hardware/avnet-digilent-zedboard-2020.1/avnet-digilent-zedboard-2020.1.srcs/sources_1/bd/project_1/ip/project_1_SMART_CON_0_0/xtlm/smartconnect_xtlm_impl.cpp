...
target_rd_util::target_rd_util(sc_core::sc_module_name p_name,
                               xtlm::aximm::granularity g_hint,
                               int width_p,
                               int si_number_,
                               smartconnect_xtlm *_sxtlm_ptr)
    : si_number(si_number_),
      xtlm::xtlm_aximm_target_rd_socket_util(p_name, g_hint, width_p),
      sxtlm_ptr(_sxtlm_ptr) {}
unsigned int
target_rd_util::transport_dbg_cb(xtlm::aximm_payload &trans) {
  return sxtlm_ptr->transport_dbg_cb(trans, si_number);
}
void target_rd_util::b_transport(xtlm::aximm_payload &trans,
                                 sc_core::sc_time &t) {
  ...
  /*
          ...
          if (parent->m_report_handler->get_verbosity_level()
                  ...
                  ss << parent->m_module_name << ": "
                                  << "Received blocking READ  call
     b_transport IN sock_id "
                                  << m_index << std::endl <<
     payloadDes; parent->m_report_handler->report("1",
     ss.str().c_str(), xsc::common_cpp::INFO, xsc::common_cpp::DEBUG);
          }
     ...
     parent->m_b_trans_master_req[master_itf_id].front() != &trans) {
                  ...
                  if (parent->m_report_handler->get_verbosity_level()
                          ...
                          ss.str("");
                          ss << parent->m_module_name << ": "
                                          << "Forwarding blocking READ
     call b_transport IN to slave "
                                          << master_itf_id <<
     std::endl << payloadDes; parent->m_report_handler->report("1",
     ss.str().c_str(), xsc::common_cpp::INFO, xsc::common_cpp::DEBUG);
                  }
...
/*
  void target_rd_util::register_b_transport(void
  (*smartconnect_xtlm::b_transport_handler_)(xtlm::aximm_payload&
  trans, sc_core::sc_time& t,int si_num)) { b_transport_handler =
  b_transport_handler_;
  }
  void target_rd_util::register_transport_dbg(unsigned int
  (*smartconnect_xtlm::transport_dbg_handler_)(xtlm::aximm_payload&
  trans,int si_num)) { transport_dbg_handler = transport_dbg_handler_;
  }
  */
target_wr_util::target_wr_util(sc_core::sc_module_name p_name,
    ...
    : si_number(si_number_),
      ...
      sxtlm_ptr(_sxtlm_ptr) {}
unsigned int
target_wr_util::transport_dbg_cb(xtlm::aximm_payload &trans) {
  return sxtlm_ptr->transport_dbg_cb(trans, si_number);
}
void target_wr_util::b_transport(xtlm::aximm_payload &trans,
                                 sc_core::sc_time &t) {
  sxtlm_ptr->b_transport_cb(trans, t, si_number);
  /*
     ...
     parent->m_b_trans_master_req[master_itf_id].front() != &trans) {
                  ...
                  if (parent->m_report_handler->get_verbosity_level()
                          ...
                          ss << parent->m_module_name << ": "
                                          << "Forwarding blocking
     WRITE call b_transport IN to slave "
                                          << master_itf_id <<
     std::endl << payloadDes; parent->m_report_handler->report("1",
     ss.str().c_str(), xsc::common_cpp::INFO, xsc::common_cpp::DEBUG);
                  }
...
/*
  void target_wr_util::register_b_transport(void
  (smartconnect_xtlm::*b_transport_handler_)(xtlm::aximm_payload&
  trans, sc_core::sc_time& t, int si_num)) { b_transport_handler =
  b_transport_handler_;
  }
  void target_wr_util::register_transport_dbg(unsigned int
  (smartconnect_xtlm::*transport_dbg_handler_)(xtlm::aximm_payload&
  trans,int si_num)) { transport_dbg_handler = transport_dbg_handler_;
  }
  */
