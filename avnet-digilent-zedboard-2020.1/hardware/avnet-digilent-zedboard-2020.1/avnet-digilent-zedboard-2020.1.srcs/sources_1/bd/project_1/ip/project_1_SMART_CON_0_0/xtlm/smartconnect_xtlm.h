...
public:
  smartconnect_xtlm(sc_core::sc_module_name name,
                    ...
                    map<string, string> _smc_properties,
...
private:
  ...
  map<xtlm::aximm_payload *, xtlm::aximm_payload *>
      map_wr_mi_to_si; // Maps generated MI trans to incoming SI trans
  map<xtlm::aximm_payload *, int>
      map_wr_si_to_nummi; // Maps each incoming SI trans to number of
                          // MI trans generated
  map<xtlm::aximm_payload *, int>
      map_wr_si_to_numsi; // Maps each incoming SI trans to
                          // corresponding SI port
  multimap<xtlm::aximm_payload *, xtlm::aximm_payload *>
      mmap_wr_si_to_mi; // Maps each incoming SI trans to multiple MI
                        // trans generated
  multimap<xtlm::aximm_payload *, xtlm::aximm_payload *>::iterator
      mmap_wr_si_to_mi_itr;

  map<xtlm::aximm_payload *, xtlm::aximm_payload *>
      map_rd_mi_to_si; // Maps generated MI trans to incoming SI trans
  map<xtlm::aximm_payload *, int>
      map_rd_si_to_nummi; // Maps each incoming SI trans to number of
                          // MI trans generated
  map<xtlm::aximm_payload *, int>
      map_rd_si_to_numsi; // Maps each incoming SI trans to
                          // corresponding SI port
  multimap<xtlm::aximm_payload *, xtlm::aximm_payload *>
      mmap_rd_si_to_mi; // Maps each incoming SI trans to multiple MI
                        // trans generated
  multimap<xtlm::aximm_payload *, xtlm::aximm_payload *>::iterator
  ...
  void print_log(std::string port,
                 int port_id,
                 std::string rw,
                 std::string action,
                 xtlm::aximm_payload *&trans_ptr);

  // To check whether it is legal address
  bool is_address_hit(xtlm::aximm_payload *&trans_ptr,
                      int num_si,
  ...
  transaction_status
  get_transaction_status(xtlm::aximm_payload *&trans_ptr,
                         int num_si,
                         unsigned long long &si_sep_route,
                         int &m_burst_length,
                         unsigned int &m_burst_size);
  ...
  void
  create_burst_transaction(std::list<xtlm::aximm_payload *> &saxi_vec,
                           std::list<xtlm::aximm_payload *> &maxi_vec,
                           std::list<unsigned int> &axiid_vec,
                           int m_burst_length,
                           unsigned int m_burst_size,
                           int &req_cnt,
                           unsigned int numsi,
  ...
  sc_core::sc_event check_saxi_wr_req, check_saxi_rd_req,
      send_wr_resp, initiate_maxi_wr_req, initiate_maxi_rd_req,
      send_saxi_rd_resp;
  ...
  // Populated from smc_properties (until xtlm handoff includes user
  // parameters)
  int smc_num_si;
  int smc_num_mi;
...
// Extension class to carry SEP_ROUTE information in cascaded
// smartconnect topology
class smartconnect_extension
    : public xtlm::xtlm_extension<smartconnect_extension> {
public:
  smartconnect_extension()
      : xtlm::xtlm_extension<smartconnect_extension>() {
    sep_route = 0;
  }

  virtual xtlm::xtlm_extension_base *clone() const { return NULL; }
  ...
  unsigned long long get_sep_route() { return sep_route; }
};
#endif
