...
def read_packages(xilinx_arch):
    ...
    return passwd,autologin,image_classes,package_feeds,image_packages,inherit_packages,image_features,mali_backend,extra_users
...
def update_cfg(cfg_file,xilinx_arch,proot):
    ...
    cfg_file.truncate()
    passwd,autologin,image_classes,package_feeds,image_packages,inherit_packages,image_features,mali_backend,extra_users=read_packages(xilinx_arch)
    # Add inherit packages into bb file
    cfg_file.write('\n#Rootfs configs\n')
    inherit_str='INHERIT += "plnx-deploy extrausers ' + ' '.join(inherit_packages) + '"\n'
    cfg_file.write(inherit_str)

    # Add image inherit packages
    if image_classes:
        image_classes='\nIMAGE_CLASSES += "' + ' '.join(image_classes) + '"\n'
        cfg_file.write(image_classes)

    # Add autologin variable
    cfg_file.write('\nIMAGE_AUTOLOGIN = "' + str(autologin) + '"\n\n')
    # Add common features into bb file
    cfg_file.write('COMMON_FEATURES_' + xilinx_arch + ' = "\\\n')
    write_list(cfg_file,image_features)
    cfg_file.write('IMAGE_LINGUAS_' + xilinx_arch + ' = " "\n\n')
    # Add image install packages into bb file
    cfg_file.write('IMAGE_INSTALL_' + xilinx_arch + ' = "\\\n')
    cfg_file.write("\t\tkernel-modules \\\n")
    write_list(cfg_file,image_packages)
    if package_feeds != "":
        package_feeds = 'PACKAGE_FEED_URIS = "' + package_feeds + '"\n\n'
        cfg_file.write(package_feeds)
    add_user_params(extra_users,passwd,cfg_file)
    if mali_backend != "":
        mali_backend = 'MALI_BACKEND_DEFAULT = "' + mali_backend + '"\n\n'
        cfg_file.write(mali_backend)
    cfg_file.close()
