...
proc plnx_gen_conf_memory {mapping kconfprefix cpuname cpuslaves} {
	...
	set choicestr [format "%s\n\t%s\n\t%s\n\t%s\n\t%s\n\t%s\n\t%s\n\t%s\n\t%s\n%s\n%s\n" \
		...
		"  you will need to specify u-boot text base address in u-boot config file." \
		"${choicestr}" \
		"endchoice"]
...
proc plnx_gen_conf_serial {mapping kconfprefix cpuname cpuslaves} {
	...
	foreach m ${mapping} {
		...
		foreach hd ${hds} {
			...
			if {"${baudrateproperty}" != "" && "${is_baudrate_editable}" == "n"} {
				...
				set baudrates {600 9600 28800 115200 230400 460800 921600}
			}
	...
	} elseif { [regexp "ps7_cortexa9*" $cpuname matched] == 1 } {
		set components_list "FSBL DTG"
	} elseif { [regexp "microblaze*" $cpuname matched] == 1 } {
	...
	foreach component $components_list {
		...
		foreach str $tmpstr {
			...
			if { "${component}" == "ATF" } {
				...
				} elseif { "${str}" == "psu_uart_1"} {
					set atf_console "cadence1"
				} elseif { "${str}" == "psv_sbsauart_0"} {
					set atf_console "pl011"
				} elseif { "${str}" == "psv_sbsauart_1"} {
...
proc plnx_gen_hwsysconf {args} {
	...
	set hwmenustr [format "%s\t%s\n" "${hwmenustr}" "  If SUBSYSTEM_AUTOCONFIG_DTS is selected, the configuration will be applied to DTS generation;"]
